 ...
 **/
Eq1Op::Eq1Op(SpEOMatrixD* R_m, SpEOMatrixD gauss_filter_m, SpEOMatrixD filter_coeff_m, int fDS_m, double coeff1_m, double coeff2_m, double coeff3_m) {
  R = R_m;
...
void Eq1MVar::SubtractOperatorXvar(Eq1Op op, SpEOMatrixD* m_var,    MPI_Comm eq1_comm) {
  int channel_X, channel_Y;
  SpEOMatrixD* TMP = new SpEOMatrixD[N_center];
  for (channel_X = 0; channel_X < N_center; channel_X++) {
     TMP[channel_X] = SpEOMatrixD::Zero((center[channel_X]).rows(),(center[channel_X]).cols());
  }
  for (channel_Y = 0; channel_Y < N_upper; channel_Y++) {
    ...
    // calculate R*var
    for (channel_X = 0; channel_X < N_center; channel_X++) {
      TMP[channel_X] += op.coeff2*((*op.R)(channel_X,channel_Y))*(m_var[channel_Y]);
    }
    // calculate var*BS
    SpEOMatrixD tmp_image_HR = m_var[channel_Y];
    SpEOMatrixD tmp_image_LR;
    if (op.fastfilter == true) {
      fast_filter(tmp_image_LR, tmp_image_HR, op.gauss_filter, op.filter_coeff, op.fDS);
    }
    else {
      standard_filter(tmp_image_LR, tmp_image_HR, op.gauss_filter, op.filter_coeff, op.fDS);
    }
    lower[channel_Y] -= op.coeff3*tmp_image_LR;
    
  }
  for (channel_X = 0; channel_X < N_center; channel_X++) {
     MPI_Allreduce(MPI_IN_PLACE,TMP[channel_X].data(),(center[channel_X]).rows()*(center[channel_X]).cols(),MPI_DOUBLE,MPI_SUM,eq1_comm);
 ...
 void Eq1MVar::SetOperatorXvar(Eq1Op op, Eq1SVar* m_var,    MPI_Comm eq1_comm) {
  int channel_X, channel_Y;
  for (channel_X = 0; channel_X < N_center; channel_X++) {
      center[channel_X] = SpEOMatrixD::Zero((center[channel_X]).rows(),(center[channel_X]).cols());
  }
  for (channel_Y = 0; channel_Y < N_upper; channel_Y++) {
    upper[channel_Y] = op.coeff1*(m_var->var[channel_Y]);
    // calculate R*var
    for (channel_X = 0; channel_X < N_center; channel_X++) {
      center[channel_X] += op.coeff2*((*op.R)(channel_X,channel_Y))*(m_var->var[channel_Y]);
    }
    // calculate var*BS
  ...
  for (channel_X = 0; channel_X < N_center; channel_X++) {
     MPI_Allreduce(MPI_IN_PLACE,center[channel_X].data(),(center[channel_X]).rows()*(center[channel_X]).cols(),MPI_DOUBLE,MPI_SUM,eq1_comm);
  }
  return;
}

   void Eq1MVar::SubtractOperatorXvar_sub(Eq1Op op, SpEOMatrixD* m_var_sub,MPI_Comm eq1_comm, SpEOMatrixD &EndmemberMat, SpEOVectorD &LAMBDA_ZY) {
  int channel_X, channel_Y;
  ...
  return;
   ...
   void Eq1MVar::SetOperatorXvar(Eq1Op op, Eq1SVar_sub* m_var_sub,MPI_Comm eq1_comm, SpEOMatrixD &EndmemberMat, SpEOVectorD &LAMBDA_ZY) {
  int channel_X, channel_Y;
  SpEOMatrixD *m_var = new SpEOMatrixD[N_upper];
  bool transpose_2D_mat = false;


  multiply_2D_with_3D_mat(m_var,EndmemberMat,m_var_sub->var,transpose_2D_mat);
  ...
  for (channel_Y = 0; channel_Y < N_upper; channel_Y++) {
    ...
    // calculate R*var
    for (channel_X = 0; channel_X < N_center; channel_X++) {
      center[channel_X] += op.coeff2*((*op.R)(channel_X,channel_Y))*(m_var[channel_Y]);
    }
    // calculate var*BS
    SpEOMatrixD tmp_image_HR = m_var[channel_Y];
    SpEOMatrixD tmp_image_LR;
...
void Eq1SVar::SetAdjointOperatorXvar(Eq1Op op, Eq1MVar* m_var) {
  ...
  for (channel_Y = 0; channel_Y < m_var->N_upper; channel_Y++) {
    ...
    for (channel_X = 0; channel_X < m_var->N_center; channel_X++) {
      var[channel_Y] += op.coeff2*((*op.R)(channel_X,channel_Y))*(m_var->center[channel_X]);
    }
    SpEOMatrixD tmp_image_LR = m_var->lower[channel_Y];
    SpEOMatrixD tmp_image_HR;
    standard_adjoint_filter(tmp_image_HR, tmp_image_LR, op.gauss_filter, op.filter_coeff, op.fDS);
...
void Eq1SVar_sub::SetAdjointOperatorXvar(Eq1Op op, Eq1MVar* m_var, SpEOMatrixD &EndmemberMat, SpEOVectorD &LAMBDA_ZY) {
  int channel_X, channel_Y;
  SpEOMatrixD *var_tmp = new SpEOMatrixD[m_var->N_upper];
  for (channel_Y = 0; channel_Y < m_var->N_upper; channel_Y++) {
    var_tmp[channel_Y] = SpEOMatrixD::Zero(m_var->upper[0].rows(),m_var->upper[0].cols());
  }
  for (channel_Y = 0; channel_Y < m_var->N_upper; channel_Y++) {
    var_tmp[channel_Y] = op.coeff1*(m_var->upper[channel_Y])*LAMBDA_ZY(channel_Y);
    for (channel_X = 0; channel_X < m_var->N_center; channel_X++) {
      var_tmp[channel_Y] += op.coeff2*((*op.R)(channel_X,channel_Y))*(m_var->center[channel_X]);
    }
    SpEOMatrixD tmp_image_LR = m_var->lower[channel_Y];
    SpEOMatrixD tmp_image_HR;
    standard_adjoint_filter(tmp_image_HR, tmp_image_LR, op.gauss_filter, op.filter_coeff, op.fDS);
...
void multiply_2D_with_3D_mat(SpEOMatrixD* &Mat_3D_output ,SpEOMatrixD &Mat_2D_input, SpEOMatrixD* &Mat_3D_input, bool transpose_2D_mat){
  ...
  for(channelU=0; channelU<Mat_2D.rows(); channelU++){
    Mat_3D_output[channelU] = SpEOMatrixD::Zero(Mat_3D_input[0].rows(), Mat_3D_input[0].cols());
    for(channelV=0; channelV<Mat_2D.cols(); channelV++){
      Mat_3D_output[channelU] += Mat_2D(channelU, channelV)*Mat_3D_input[channelV];
    }
  }
}
...
double Eq6SVar::NormSquared() {
  return pow(var.norm(),2);
}

void Eq6SVar::SetAdjointOperatorXvar(Eq6Op op, Eq6MVar* m_var) {
  var = op.coeff1*m_var->upper + op.coeff2*((*op.R).transpose()*m_var->lower);
  return;
}
 ...
 **/
Eq7Op::Eq7Op(SpEOVectorD *dH_m, SpEOVectorD *dL_m, SpEOMatrixD* R_m, SpEOVectorD* P_lmd_vecs_m, SpEOMatrixI* P_lmd_idx_row_m, int **P_lmd_idx_bl_m, double *coeff1_m, double *coeff2_m, double coeff3_m) {
  dH = dH_m;
  dL = dL_m;
  R = R_m;
...
void Eq7MVar::SubtractOperatorXvar(Eq7Op op, SpEOVectorD* m_var) {
  ...
  int running_idx = 0;
  for (g = 0; g < N_g; g++) {
    tmpDA.block(running_idx,0,op.P_lmd_vecs[g].size(),N_h) = (op.dH[g]*m_var[g].transpose()).transpose();
  ...
  for (j = 0; j < N_Y; j++) {
    for (k = 0; k < op.P_lmd_idx_row[j].cols(); k++) {
      ...
      tmpPDA.row(j) += op.P_lmd_vecs[block](relidx)*tmpDA.row(op.P_lmd_idx_bl[block][1] + relidx);
    }
  }
...
void Eq7MVar::SetOperatorXvar(Eq7Op op, Eq7SVar* m_var) {
  ...
  int running_idx = 0;
  for (g = 0; g < N_g; g++) {
    tmpDA.block(running_idx,0,op.P_lmd_vecs[g].size(),N_h) = (op.dH[g]*m_var->var[g].transpose()).transpose();
    running_idx += op.P_lmd_vecs[g].size();
  }
  SpEOMatrixD tmpPDA = SpEOMatrixD::Zero(N_Y,N_h);
  for (j = 0; j < N_Y; j++) {
...
void Eq7SVar::SetAdjointOperatorXvar(Eq7Op op, Eq7MVar* m_var) {
  ...
  for (g = 0; g < N_g; g++) {
    var[g] = (op.coeff1[g] * (op.dH[g].transpose()*m_var->upper[g]) + op.coeff2[g] * (op.dL[g].transpose()*m_var->center[g])).transpose();
    SpEOMatrixD temp = SpEOMatrixD::Zero(N_h,op.P_lmd_vecs[g].size());
    for (j = 0; j < op.P_lmd_vecs[g].size(); j++) { // write temp = B^t*P(k)
      temp.col(j) = (op.P_lmd_vecs[g](j))*tempR.row(op.P_lmd_idx_bl[g][0]+j).transpose();
    }
 ...
 **/
Eq9Op::Eq9Op(SpEOMatrixD *DH_m, SpEOMatrixD *DL_m, SpEOMatrixD* R_m, SpEOVectorD* P_lmd_vecs_m, SpEOMatrixI* P_lmd_idx_row_m, int **P_lmd_idx_bl_m, double *coeff1_m, double *coeff2_m, double coeff3_m) {
  DH = DH_m;
  DL = DL_m;
  R = R_m;
  P_lmd_vecs = P_lmd_vecs_m;
...
void Eq9MVar::SubtractOperatorXvar(Eq9Op op, SpEOMatrixD* m_var) {
  ...
  for (g = 0; g < N_g; g++) {
    ...
    upper[g] -= op.coeff1[g]*(op.DH[g].rightCols(N_DP-1)*m_var[g].bottomRows(N_DP-1));
    center[g] -= op.coeff2[g]*(op.DL[g].rightCols(N_DP-1)*m_var[g].bottomRows(N_DP-1));
  }
  ...
  for (g = 0; g < N_g; g++) {
    int N_DP = op.DH[g].cols();
    tmpDA.block(running_idx,0,op.P_lmd_vecs[g].size(),N_h) = (op.DH[g].rightCols(N_DP-1)*m_var[g].bottomRows(N_DP-1)).transpose();
    running_idx += op.P_lmd_vecs[g].size();
  }
  SpEOMatrixD tmpPDA = SpEOMatrixD::Zero(N_Y,N_h);
  for (j = 0; j < N_Y; j++) {
...
void Eq9MVar::SetOperatorXvar(Eq9Op op, Eq9SVar* m_var) {
  ...
  for (g = 0; g < N_g; g++) {
    int N_DP = op.DH[g].cols();
    tmpDA.block(running_idx,0,op.P_lmd_vecs[g].size(),N_h) = (op.DH[g].rightCols(N_DP-1)*m_var->var[g]).transpose();
    running_idx += op.P_lmd_vecs[g].size();
  }
  SpEOMatrixD tmpPDA = SpEOMatrixD::Zero(N_Y,N_h);
  for (j = 0; j < N_Y; j++) {
...
void Eq9SVar::SetAdjointOperatorXvar(Eq9Op op, Eq9MVar* m_var) {
  ...
  SpEOMatrixD tempR = (*op.R).transpose()*m_var->lower;
  for (g = 0; g < N_g; g++) {
    int N_DP = op.DH[g].cols();var[g] = op.coeff1[g] * (op.DH[g].rightCols(N_DP-1).transpose()*m_var->upper[g]) + op.coeff2[g] * (op.DL[g].rightCols(N_DP-1).transpose()*m_var->center[g]);
    SpEOMatrixD temp = SpEOMatrixD::Zero(N_h,op.P_lmd_vecs[g].size());
    for (j = 0; j < op.P_lmd_vecs[g].size(); j++) { // write temp = B^t*P(k)
      temp.col(j) = (op.P_lmd_vecs[g](j))*tempR.row(op.P_lmd_idx_bl[g][0]+j).transpose();
    }
...
void softThresholdJS(double alpha, SpEOMatrixD * A, SpEOMatrixD& x, SpEOMatrixD &Ax) {
  int i;
  for (i = 0; i < x.rows(); i++) {
    double x_abs = x.row(i).norm();
    if (x_abs > alpha) {
...
void softThresholdJS_Phit(double alpha, SpEOMatrixD * AtA, SpEOMatrixD& x, SpEOMatrixD &AtAx) {
  int i;
  for (i = 0; i < x.rows(); i++) {
    double x_abs = x.row(i).norm();
    if (x_abs > alpha) {
...
double J_1D(double x, SpEOMatrixD Ad_h, SpEOMatrixD y_Ab, SpEOMatrixD d_h, SpEOMatrixD b_h, double tau) {
  return 0.5*pow((x*Ad_h - y_Ab).norm(),2) + tau*l1l2norm(x* d_h + b_h);
}

double J_1D(double x, double Ad_h_sq, double Ad_h_y_Ab, double y_Ab_sq, SpEOVectorD d_s, SpEOVectorD d_s_b_s, SpEOVectorD b_s, double tau) {
  return 0.5*(Ad_h_sq*pow(x,2) - Ad_h_y_Ab*x + y_Ab_sq) + tau*(pow(x,2)*d_s + x*d_s_b_s + b_s).cwiseAbs().cwiseSqrt().sum();
}

double J_1D(double x, double Ad_h_sq, double Ad_h_y_Ab, double y_Ab_sq, SpEOVectorD d_s, SpEOVectorD d_s_b_s, SpEOVectorD b_s, double tau, MPI_Comm comm_group) {
  double commdata = (pow(x,2)*d_s + x*d_s_b_s + b_s).cwiseAbs().cwiseSqrt().sum();
...
double J_minsearch(SpEOMatrixD Ad_h, SpEOMatrixD y_Ab, SpEOMatrixD d_h, SpEOMatrixD b_h, double tau, double x0, double tol) { // Nelder Mead Simplex in 1D
  double alpha = 1; double gamma = 2; double beta = 0.5; double sigma = 0.5;
  double x1 = x0+1.0;
...
double J_minsearch(double Ad_h_sq, double Ad_h_y_Ab, double y_Ab_sq, SpEOVectorD d_s, SpEOVectorD d_s_b_s, SpEOVectorD b_s, double tau, double x0, double tol, int &iter) { // Nelder Mead Simplex in 1D
  double alpha = 1.0; double gamma = 2.0; double beta = 0.5; double sigma = 0.5;
  double x1 = x0+1.0;
...
double J_minsearch(double Ad_h_sq, double Ad_h_y_Ab, double y_Ab_sq, SpEOVectorD d_s, SpEOVectorD d_s_b_s, SpEOVectorD b_s, double tau, double x0, double tol, int &iter, MPI_Comm comm_group) { // Nelder Mead Simplex in 1D
  double alpha = 1.0; double gamma = 2.0; double beta = 0.5; double sigma = 0.5;
  double x1 = x0+1.0;
  double f0 = J_1D(x0, Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, tau, comm_group);
  double f1 = J_1D(x1, Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, tau, comm_group);
  iter = 0;
  while (abs(x1 - x0) > tol) {
    iter++;
    if (f0 > f1) { // sort
    ...
    double r = (1.0+alpha)*x0 - alpha*x1;
    double fr = J_1D(r, Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, tau, comm_group);
    if (fr < f0) {
      double e = (1.0+gamma)*x0 - gamma*x1;
      double fe = J_1D(e, Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, tau, comm_group);
      if (fr < fe) {
	x1 = r; f1 = fr;
      }
      else {
	...
	c = beta*x0 + (1.0-beta)*x1;
      ...
      double fc = J_1D(c, Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, tau, comm_group);
      if (fc < f1) {
	x1 = c; f1 = fc;
      }
      else {
	x1 = sigma*x0 + (1.0-sigma)*x1;
	f1 = J_1D(x1, Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, tau, comm_group);
      }
    }
  }
  return x0;
...
// parallel FBS
void FBSSolver(int &iter_need, double &rel_res, SpEOMatrixD &X, int N, SpEOMatrixD Phi, SpEOMatrixD y, double alpha, FBSsolverOptions opts, MPI_Comm comm_group, SpEOMatrixD &timestat, SpEOMatrixD &btstat) {
	double eta = 0.5;
	...
	timestat = SpEOMatrixD::Zero(1,5); // total, communication, backtracking, prediction, stopping criterion
	...
	if (opts.pred_rule == FISTA && L_max > 1) {
	  if (my_rank == 0 && !opts.silent) {
	    cout << "FBS Solver WARNING: Prediction Rule FISTA is only allowed with opts.maxiter_in == 1. Value is set to 1!" << endl;
	  }
	  L_max = 1;
	}
	else if (opts.stepsize_update == PSCL && L_max > 1) {
	  if (my_rank == 0 && !opts.silent) {
	    cout << "FBS Solver WARNING: Stepsize Update Rule PSCL is only allowed with opts.maxiter_in == 1. Value is set to 1!" << endl;
	  }
	  L_max = 1;
	}
	int P = opts.Ndecomp;
	if (P != my_processes) {
	  if (my_rank == 0 && !opts.silent) {
	    cout << "FBS Solver WARNING: P = " <<  P  << " is not equal to number of processes. P is set to " <<  my_processes << "!" << endl;
	  }
	}
	...
	while (!stop_crit && (iterations < opts.maxiter_out)) {
	      ...
	      timestat(0,0) += MPI_Wtime() - time_tmp; time_tmp = MPI_Wtime();
		...
		L = max(L, min(L_max, (int) (2 + log10((rel_res/opts.tol)/opts.gamma)*(L_max - 1.0)/(-log10(opts.gamma)))));
	      }
	      double stepsize;
	      int iter_in;
		...
		if (opts.stepsize_update == INIT) {
		  ...
		  if (iterations == 0) {
		    ...
		    double tmp_sz2 = 2*(Xtmp - Xtmp_prev).cwiseProduct(PhitPhiXtmp - PhitPhiXtmp_prev).sum();
		...
		*/
		if (opts.backtracking == DEC || opts.backtracking == DECEFF) {
		  ...
		  while (!step_cond && stepsize > 0.01 ) {
		    ...
		    if (opts.backtracking == DEC) {
		      ...
		      f_new = y_loc_norm_sq_half + X_loc.cwiseProduct(0.5*PhitPhiX_loc - Phity_loc).sum(); 
		    }
		    step_cond = f_new < 0.5*f_hat + (X_loc - Xin).cwiseProduct(-PhitPhiXin_res + 1/(2*stepsize)*(X_loc-Xin)).sum();
		  }
		  f_hat = f_new;
		  if (stepsize <= 0.01 && !step_cond && !opts.silent) {
		    cout << "FBS Solver WARNING: backtracking stepsize limit reached in iteration " << iterations << " on process with rank " << my_rank << ". Condition violation by " << 0.5*pow((y-PhiX_loc).norm(),2) - 0.5*f_hat + (X_loc - Xin).cwiseProduct(PhitPhiXin_res).sum() - 1/(2*stepsize)*pow((X_loc-Xin).norm(),2) << "." << endl;
		  }
		}
		  ...
		  while (!step_cond && stepsize > 0.01 ) {
		    ...
		    X_loc = Xin + stepsize*(PhitPhiXin_res);
		    softThresholdJS(stepsize*alpha, &Phi, X_loc, PhiX_loc);
		    double tmp_stepcond = - (X_loc - Xin).cwiseProduct(PhitPhiXin_res).sum() + 1/(2*stepsize)*pow((X_loc-Xin).norm(),2);
		    timestat(0,2) += MPI_Wtime() - time_tmp; time_tmp = MPI_Wtime();
		    MPI_Allreduce(PhiX_loc.data(),PhiX.data(),m*d,MPI_DOUBLE,MPI_SUM,comm_group);
		    MPI_Allreduce(MPI_IN_PLACE,&tmp_stepcond,1,MPI_DOUBLE,MPI_SUM,comm_group);
		    timestat(0,1) += MPI_Wtime() - time_tmp; time_tmp = MPI_Wtime();
		    step_cond = 0.5*pow((y-PhiX).norm(),2) < 0.5*f_hat + tmp_stepcond;
		  }
		  if (stepsize <= 0.01 && !step_cond && !opts.silent) {
		    cout << "FBS Solver WARNING: backtracking stepsize limit reached in iteration " << iterations << " on process with rank " << my_rank << ". " << endl;
		  }
		}
		...
		case FUNCTIONAL_DIFFERENCE:
		{
		  // update J
		  Jold = Jnew; 
		  double buf_comm = 0.5*X_loc.cwiseProduct(PhitPhiX-2*Phity).sum() + alpha*l1l2norm(X_loc);
		  MPI_Allreduce(MPI_IN_PLACE,&buf_comm,1,MPI_DOUBLE,MPI_SUM,comm_group);
		...
		case FIRST_ORDER_CONDITIONS:
		{
		  ...
		  for (i = 0; i < N_loc; i++) {
		    ...
		    if (normXnew > 1e-16) {
		      norm_squared += pow((-Phity_PhitPhiX.row(i) + (alpha/normXnew)*X_loc.row(i)).norm(),2);
		    }
		    else {
		      norm_squared += pow(max(0.0,Phity_PhitPhiX.row(i).norm() - alpha),2);
		    }
		  }
		...
		default:
		{
	      ...
	      timestat(0,4) += MPI_Wtime() - time_tmp; time_tmp = MPI_Wtime();
	      /*
	       * PREDICTION
	       */
		...
		case FISTA_RESTART:
		{
		  ...
		  if (opts.pred_rule == FISTA_RESTART) {
		    double restartval = ((Xtmp - X_loc).cwiseProduct(X_loc - X_loc_prev)).sum();
		    MPI_Allreduce(MPI_IN_PLACE,&restartval,1,MPI_DOUBLE,MPI_SUM,comm_group);
		    if (restartval >= 0) {// restart
		      if (my_rank == 0 && !opts.silent) {
			cout << "FBS Solver HINT: FISTA was restarted in iteration " << iterations << endl;
		      }
		      t_prev = 1;
		    }
		  }
		...
		{
		  weight_predict = 1.0; // guaranteed to converge only for P == 1 or param.maxiter_in == 1
		  break;	
		}
		case LINE_SEARCH_SIMPLEX:
		{
		  ...
		  MPI_Allreduce(MPI_IN_PLACE,&Ad_h_sq,1,MPI_DOUBLE, MPI_SUM, comm_group);
		  MPI_Allreduce(MPI_IN_PLACE,&Ad_h_y_Ab,1,MPI_DOUBLE, MPI_SUM, comm_group);
		  ...
		  double w = J_minsearch(Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, alpha, 0, weight_eps, iter_loc, comm_group);
		...
		{
		  weight_predict = 1.0/P;
		  double rho = 0.5; double sigma = 0.5; double eta_armijo = 2.0;
		  SpEOMatrixD d_h = X_loc - Xtmp;
		  SpEOMatrixD b_h = Xtmp;
		  ...
		  double allred_result = 2*alpha*(l1l2norm(b_h + 1.0/P*d_h) - l1l2b_h);
		  MPI_Allreduce(MPI_IN_PLACE,&allred_result,1,MPI_DOUBLE, MPI_SUM, comm_group);
		  ...
		  while (armijo_cond == false && step_armijo < max_armijo) {
		    double allred_result_loc = - 2*alpha*l1l2norm(b_h + weight_predict*d_h) + 2*alpha*l1l2b_h;
		    MPI_Allreduce(MPI_IN_PLACE,&allred_result_loc,1, MPI_DOUBLE, MPI_SUM, comm_group);
		    if(pow(weight_predict,2)*Ad_h_sq - weight_predict*Ad_h_y_Ab <= sigma*weight_predict*allsum + allred_result_loc) {
		      armijo_cond = true;
		    }
		    else {
		      weight_predict *= rho;
		    ...
		    if (weight_predict < 0.0001/P) {
		      if (my_rank == 0 && !opts.silent) {
			cout << "FBS Solver WARNING: ARMIJO stepsize very small. Value is reset to 1/P!" << endl;
		      }
		      weight_predict = 1.0/P;
		      armijo_cond == true;
		    }
		...
		default:
		{
	      ...
	      timestat(0,3) += MPI_Wtime() - time_tmp; time_tmp = MPI_Wtime();
	      // update Xtmp	      
		...
		PhiXtmp_loc = weight_predict*PhiX_loc + (1-weight_predict)*PhiX_loc_prev;
		PhitPhiXtmp_loc = weight_predict*PhitPhiX_loc + (1-weight_predict)*PhitPhiX_loc_prev;
		PhitPhiXtmp = weight_predict*PhitPhiX + (1-weight_predict)*PhitPhiX_prev;
	      }
	      else {
		...
		PhiXtmp_loc = weight_predict*PhiX_loc + (1-weight_predict)*PhiXtmp_loc;
		PhitPhiXtmp_loc = weight_predict*PhitPhiX_loc + (1-weight_predict)*PhitPhiXtmp_loc;
		PhitPhiXtmp_prev = PhitPhiXtmp; // only PSCL
		PhitPhiXtmp = weight_predict*PhitPhiX + (1-weight_predict)*PhitPhiXtmp_prev;
	      }
	      
	      if (opts.backtracking == DEC || opts.backtracking == DECPAR || opts.backtracking == DECEFF) {
	...
	// result
	X = Xtmp;
	timestat(0,0) += timestat(0,1) + timestat(0,2) + timestat(0,3) + timestat(0,4);
	return;
}
...
void FBSSolver(int &iter_need, double &rel_res, SpEOMatrixD &X, SpEOMatrixD Phi, SpEOMatrixD y, double alpha, FBSsolverOptions opts, SpEOMatrixD &timestat, SpEOMatrixD &btstat) {
	...
	if ((opts.pred_rule == FISTA || opts.pred_rule == FISTA_RESTART) && L > 1) {
	  if (!opts.silent) {
	    cout << "FBS Solver WARNING: Prediction Rule FISTA is only allowed with maxiter_in == 1. Value is set to 1!" << endl;
	  }
	  L = 1;
	}
	...
	if (opts.Ndecomp > 1) {
	  cout << "FBS Solver ERROR: This is the sequential version of the solver. Ndecomp has to be set to 1" << endl;
	  exit(EXIT_FAILURE);
	}
	...
	while (!stop_crit && (iterations < opts.maxiter_out)) {	      
		...
		L = max(L, min(opts.maxiter_in, (int) (2 + log10((rel_res/opts.tol)/opts.gamma)*(opts.maxiter_in - 1.0)/(-log10(opts.gamma)))));
	      }
		...
		if (opts.stepsize_update == INIT) {
		  ...
		  if (iter_in == 0) {
		    if (iterations == 0) {
		      ...
		      double tmp_sz2 = 2*(Xtmp - Xtmp_old).cwiseProduct(PhitPhiXtmp - PhitPhiXtmp_all_old).sum();
		      Xtmp_old = Xtmp;
		    ...
		    double tmp_sz2 = 2*(Xin - Xin_old).cwiseProduct(PhitPhiXin - PhitPhiXin_old).sum();
		    stepsize_init = max(1.7*tmp_sz1/tmp_sz2,1.0);
		  }
		}
		...
		*/
		if (opts.backtracking == DEC || opts.backtracking == DECPAR || opts.backtracking == DECEFF) {
		  ...
		  while (!step_cond && stepsize_valid > 0.01 ) {
		    ...
		    step_cond = 0.5*pow((y-PhiXnew).norm(),2) < 0.5*f_hat - (Xnew - Xin).cwiseProduct(PhitPhiXin_res).sum() + 1/(2*stepsize_valid)*pow((Xnew-Xin).norm(),2);
		  }
		  if (stepsize_valid <= 0.01 && !step_cond && !opts.silent) {
		    cout << "FBS Solver WARNING: backtracking stepsize limit reached in iteration " << iterations << ". Condition violation by " << 0.5*pow((y-PhiXnew).norm(),2) - 0.5*f_hat + (Xnew - Xin).cwiseProduct(PhitPhiXin_res).sum() - 1/(2*stepsize_valid)*pow((Xnew-Xin).norm(),2) << "." << endl;
		  }
		}
		else {
		  // Bregman step
		...
		case FIRST_ORDER_CONDITIONS:{
		  ...
		  for (i = 0; i < N; i++) {
		    ...
		    if (normXnew > 1e-16) {
		      norm_squared += pow((-PhitPhiXnew_res.row(i) + (alpha/normXnew)*Xnew.row(i)).norm(),2);
		    }
		    else {
		      norm_squared += pow(max(0.0,PhitPhiXnew_res.row(i).norm() - alpha),2);
		    }
		  }
		...
		case FISTA_RESTART: // guaranteed to converge only for param.maxiter_in == 1
		{
		  told = tnew;
		...
		case STANDARD_AGGRESSIVE:
		{
		  weight_predict = 1.0; // guaranteed to converge only for Ndecomp == 1 or param.maxiter_in == 1
		  break;	
		}
		case LINE_SEARCH_SIMPLEX:
		{
		  ...
		  SpEOVectorD d_s_b_s = 2*d_h.cwiseProduct(b_h).rowwise().sum();
		  int iter_loc;
		  double w = J_minsearch(Ad_h_sq, Ad_h_y_Ab, y_Ab_sq, d_s, d_s_b_s, b_s, alpha, 0, weight_eps, iter_loc);
		  if (w > 0) {
		    weight_predict = w;
		  }
		...
		case LINE_SEARCH_ARMIJO:
		{
		  ...
		  double l1l2b_h = l1l2norm(b_h);
		  double allred_result = 2*alpha*(l1l2norm(b_h + 1.0/Ndecomp*d_h) - l1l2b_h);
		...
		PhitPhiXtmp = weight_predict*PhitPhiXnew + (1-weight_predict)*PhitPhiXnew_old;
		Xnew_old = Xnew;
		PhiXnew_old = PhiXnew;
		...
		PhitPhiXtmp = weight_predict*PhitPhiXnew + (1-weight_predict)*PhitPhiXtmp;
	      }
...
void solve_equation3(int &iter, double &rel_res, SpEOMatrixD &Z, SpEOVectorD m, SpEOVectorD &m_out, SpEOMatrixD *DH, SpEOMatrixD *DL, SpEOVectorD* P_lmd_vecs, SpEOMatrixI* P_lmd_idx_row, int **P_lmd_idx_bl, SpEOMatrixD R, SpEOMatrixD X, SpEOMatrixD* Y, SpEOMatrixD *A, SpEOMatrixD* &A_out, SpEOMatrixD Z0, double lambda_X, double lambda_Y, int N_g, int maxiter, double tol_r, int fix_Alpha, bool fix_delta_m) {
  ...
  if (fix_Alpha == 1) {
    ...
    int running_idx = 0;
    for (k = 0; k< N_g; k++) {
      tmpDA.block(running_idx,0,A[k].cols(),N_h) = (DH[k]*A[k]).transpose();
  ...
  if (use_starting_value) { // d -= operator * x (only take the A's into account since delta_m_0 = 0, Z_0 = 0)
    d_upper -= coeff1*Z0;
  ...
  for (k=0; k < N_g; k++) {
    ...
    for (j = 0; j < A[k].cols(); j++) { // write temp = B^t*P(k)
      temp.col(j) = (P_lmd_vecs[k](j))*d_upper.row(P_lmd_idx_bl[k][0]+j).transpose();
    }
    r_center[k] = - coeff1 * (DH[k].transpose()*temp) + coeff3 * (DL[k].transpose()*d_lower2[k]);
    if (fix_Alpha == 2) {
  ...
  // compute r = operator.transposed()*d;
  SpEOMatrixD r_upper_tmp = coeff1*d_upper_tmp + coeff2*(R.transpose()*d_lower_tmp);
  SpEOMatrixD *r_center_tmp = new SpEOMatrixD[N_g];
  for (k=0; k < N_g; k++) {
    SpEOMatrixD temp = SpEOMatrixD::Zero(N_h,A[k].cols());
    for (j = 0; j < A[k].cols(); j++) { // write temp = B^t*P(k)
      temp.col(j) = (P_lmd_vecs[k](j))*d_upper_tmp.row(P_lmd_idx_bl[k][0]+j).transpose();
    }
    r_center_tmp[k] = - coeff1 * (DH[k].transpose()*temp) + coeff3 * (DL[k].transpose()*d_lower2_tmp[k]);
  }
  ...
  if (fix_Alpha == 0 || fix_Alpha == 2) {
    ...
    int running_idx = 0;
    for (k = 0; k< N_g; k++) {
      tmpDA.block(running_idx,0,A[k].cols(),N_h) = (DH[k]*p_center[k]).transpose();
      running_idx += A[k].cols();
    }
    SpEOMatrixD tmpPDA = SpEOMatrixD::Zero(N_Y,N_h);
    for (j = 0; j < N_Y; j++) {
  ...
  while (iter < maxiter && sqrt(r_norm_squared)/r0_norm > tol_r) {
    ...
    alpha = r_norm_squared/(pow(t_lower.norm(),2) + pow(t_upper.norm(),2) + t_lower2_sq);
  ...
  if(my_rank==0){cout << "resudial in all iterations:" << endl;}
  iter = 0;
  while (iter < maxiter && sqrt(r_norm_squared)/r0_norm > tol_r) {
     if(my_rank==0){cout << sqrt(r_norm_squared)/r0_norm <<  endl;}
  ...
  if (use_starting_value) { 
    // d =  d - operator * x
    d.SubtractOperatorXvar_sub(op, AbundanceMat, eq1_comm, EndmemberMat, LAMBDA_ZY);
  }
  ...
  iter = 0;
  if(my_rank==0){cout << "residual in all iterations: " << endl;}
  while (iter < maxiter && sqrt(r_norm_squared)/r0_norm > tol_r) {
     if(my_rank==0){cout << sqrt(r_norm_squared)/r0_norm <<  endl;}
    // alpha = (norm(r)/norm(t))^2
    alpha = r_norm_squared/t.NormSquared(eq1_comm);
...
//  int i;
  ...
  double r_resp = 0.0;
  r_resp += pow((coeff1*B_upper + coeff2*(R.transpose()*B_lower)).norm(),2);
  if (!fix_Alpha) {
    for (k=0; k < N_g; k++) {
      SpEOMatrixD temp = SpEOMatrixD::Zero(N_h,A[k].cols());
      for (j = 0; j < A[k].cols(); j++) { // write temp = B^t*P(k)
	temp.col(j) = (P_lmd_vecs[k](j))*B_upper.row(P_lmd_idx_bl[k][0]+j).transpose();
      }
      r_resp += pow((- coeff1 * (DH[k].transpose()*temp) + coeff3 * (DL[k].transpose()*B_lower2[k])).norm(),2);
    }
  }
  if (!fix_delta_m) {
  ...
  while (iter < maxiter && diff/r_resp > tol_r) {
    ...
    if (!fix_Alpha) {
      ...
      int running_idx = 0;
      for (k = 0; k< N_g; k++) {
	tmpDA.block(running_idx,0,A[k].cols(),N_h) = (DH[k]*x_center[k]).transpose();
	running_idx += A[k].cols();
      }
      SpEOMatrixD tmpPDA = SpEOMatrixD::Zero(N_Y,N_h);
      for (j = 0; j < N_Y; j++) {
...
void calcOptCoeffResLS(int &iter, double &rel_res, SpEOMatrixD* &A_res, SpEOMatrixD* A_res_0, SpEOMatrixD *DH, SpEOMatrixD *DL, SpEOVectorD* m, SpEOVectorD* delta_m, SpEOMatrixD* Z, SpEOMatrixD* Y, SpEOMatrixD* X, SpEOMatrixD* R, SpEOVectorD* P_lmd_vecs, SpEOMatrixI* P_lmd_idx_row, int **P_lmd_idx_bl, double lambda_X, double lambda_Y, double lambda_Z, int N_g, int maxiter, double tol_r) {
  ...
  // define operator
  Eq9Op op = Eq9Op(DH, DL, R, P_lmd_vecs, P_lmd_idx_row, P_lmd_idx_bl, coeff1, coeff2, coeff3);
  
  // compute initial right-hand side
  Eq9MVar b = Eq9MVar(N_g);
  Eq9MVar d = Eq9MVar(N_g);
  SpEOMatrixD tmp_upper = ((*Z) - ((*m)+(*delta_m))*onesNh.transpose()).transpose();
  SpEOMatrixD tmp_center = ((*Y) - ((*m)*onesNl.transpose())).transpose();
  for (g = 0; g < N_g; g++) {
    b.upper[g] = coeff1[g] * (tmp_upper.middleCols(P_lmd_idx_bl[g][0],A_res_0[g].cols()) - DH[g].col(0)*A_res_0[g].row(0)); 
    b.center[g] = coeff2[g] * (tmp_center.middleCols(P_lmd_idx_bl[g][0],A_res_0[g].cols()) - DL[g].col(0)*A_res_0[g].row(0)); 
    // d = right-hand side
    d.upper[g] = b.upper[g];
  ...
  int running_idx = 0;
  for (g = 0; g < N_g; g++) {
    tmpDA.block(running_idx,0,A_res_0[g].cols(),N_h) = (DH[g].col(0)*A_res_0[g].row(0)).transpose();
  ...
  b.lower = coeff3 * ((*X) - (*R)*(tmpPDA + ((*m)+(*delta_m))*onesNh.transpose()));
  d.lower = b.lower;
...
void calcOptCoeffCurrPatchLS(int &iter, double &rel_res, SpEOVectorD* &a, SpEOVectorD* a0, SpEOVectorD *dH, SpEOVectorD *dL, SpEOVectorD* m, SpEOVectorD* delta_m, SpEOMatrixD* Z, SpEOMatrixD* Y, SpEOMatrixD* X, SpEOMatrixD* R, SpEOVectorD* P_lmd_vecs, SpEOMatrixI* P_lmd_idx_row, int **P_lmd_idx_bl, double lambda_X, double lambda_Y, double lambda_Z, int N_g, int maxiter, double tol_r) {
  ...
  // define operator
  Eq7Op op = Eq7Op(dH, dL, R, P_lmd_vecs, P_lmd_idx_row, P_lmd_idx_bl, coeff1, coeff2, coeff3);
  ...
  SpEOMatrixD tmp_center = ((*Y) - ((*m)*onesNl.transpose())).transpose();
  for (g = 0; g < N_g; g++) {
    b.upper[g] = coeff1[g] * tmp_upper.middleCols(P_lmd_idx_bl[g][0],a0[g].size()); 
    b.center[g] = coeff2[g] * tmp_center.middleCols(P_lmd_idx_bl[g][0],a0[g].size()); 
    // d = right-hand side
    d.upper[g] = b.upper[g];
    d.center[g] = b.center[g];
  }
...
void calcOptCoeffResPFISTA(int* &iter, double* &rel_res, SpEOMatrixD* &A, SpEOMatrixD* A_0, SpEOMatrixD *DH, SpEOMatrixD *DL, SpEOVectorD* m, SpEOVectorD* delta_m, SpEOMatrixD* Z, SpEOMatrixD* Y, int **P_lmd_idx_bl, double lambda_A, double lambda_Y, double lambda_Z, int N_g, FBSsolverOptions opts, bool spectral_normalizer, bool &write_testset, int testnr) {
  ...
  for (g = 0; g < N_g; g++) {
    ...
    double coeff1 = sqrt(lambda_Z/(N_h*N_c)); // exclude 1/2 in order to fulfill standard formulation
    double coeff2 = sqrt(lambda_Y/(N_l*N_c)); // exclude 1/2 in order to fulfill standard formulation
    SpEOMatrixD y = SpEOMatrixD::Zero(N_h + N_l, N_c);
    y.topRows(N_h) = coeff1 * (tmp_upper.middleCols(P_lmd_idx_bl[g][0], N_c) - DH[g].col(0)*A_0[g].row(0)); 
    y.bottomRows(N_l) = coeff2 * (tmp_center.middleCols(P_lmd_idx_bl[g][0], N_c) - DL[g].col(0)*A_0[g].row(0)); 
    ...
    if (spectral_normalizer) { // compute spectral norm
      normalizer = 1.5 * spec_norm(D); // use the factor 1.5 in order to make sure that the normalizer is large enough, since we only compute an approximation of the spectral norm
    }
...
SpEOMatrixD optMeanDiffLS_inverse(SpEOMatrixD* R, double lambda_X, double lambda_Z) {
  ...
  return ((lambda_Z/N_Y)*SpEOMatrixD::Identity(N_Y,N_Y) + (lambda_X/N_X)*R->transpose()*(*R)).inverse();
}
...
void calcOptMeanDiffLS_explicit(SpEOVectorD &delta_m, SpEOMatrixD* Inv, SpEOMatrixD* R, SpEOVectorD* m, SpEOMatrixD* Z, SpEOMatrixD* X, double lambda_X, double lambda_Z) {
  ...
  int N_Y = R->cols();
  int N_h = Z->cols();
  SpEOVectorD RHS = (lambda_Z/N_Y)*((1.0/N_h)*Z->rowwise().sum() - (*m)) + (lambda_X/N_X)*(R->transpose()*((1.0/N_h)*X->rowwise().sum() - (*R)*(*m)));
  delta_m = (*Inv)*RHS;
  return;
}
