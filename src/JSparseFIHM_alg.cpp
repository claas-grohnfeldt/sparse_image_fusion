...
void JSparseFIHM_alg(SpEOMatrixD &EndmemberMat, SpEOMatrixD* &AbundanceMat, int iterMain, int numIterMain, SpEOPaths *paths, SpEODataIOSetting *dSet, SpEOFusionSetting *fSet, SpEOOutputSetting *oSet, SpEOSolverSetting *sSet, SpEOParallelSetting *pSet,
	...
	delete_counter(&c); // the barrier is necessary here (otherwise the counter is deleted, while others might access it)
	if(print_for_debugging && (my_rank==0)) cout << "[" << my_rank << "] bp_7" << endl;
	...
	if(print_for_debugging && (my_rank==0)) cout << "[" << my_rank << "] bp_9" << endl;
	...
	if(print_for_debugging && (my_rank==0)) cout << "[" << my_rank << "] bp_17" << endl;
	...
	if(print_for_debugging && (my_rank==0)) cout << "[" << my_rank << "] bp_18" << endl;
	if (my_rank == 0) {
	...
	if(print_for_debugging && (my_rank==0)) cout << "[" << my_rank << "] bp_21" << endl;
#endif
}
