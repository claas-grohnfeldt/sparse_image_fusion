...
int main(int argc, char *argv[]) {
  ...
  getUserSettings(&paths, &dSetting, &fSetting, &oSetting, &sSetting, &pSetting,
                  argc, argv);
  ...
  SpEOReport report;
  if (my_rank == 0) {
    report.initialize(&paths, &dSetting, &fSetting, &oSetting, &sSetting,
                      &pSetting, argc, argv);
  }
  ...
  //=======================================================================================//
  //  Read subspace transformation matrix file (e.g. endmember matrix in case of
  //  unmixing) //
  //=======================================================================================//
  ...
  if (fSetting.subspace_transform_type == "none") {
    SubspaceTransformMat =
        SpEOMatrixD::Identity(ImY->get_NCh(), ImY->get_NCh());
  } else if (fSetting.subspace_transform_type == "SVD") {
    SpEOMatrixD ImY_2D =
        SpEOMatrixD::Zero(ImY->get_NCh(), ImY->get_sizeU() * ImY->get_sizeV());
    ...
    cerr << endl
         << "unknown subspace Transformation Type: "
         << fSetting.subspace_transform_type << endl;
    exit(2);
  }

  glPrms.NChY_subspace = SubspaceTransformMat.rows();
  // declare, initialize and possibly load the initial value of the abundance
  // matrix
  ...
  for (int iChY_sub = 0; iChY_sub < glPrms.NChY_subspace; iChY_sub++) {
    AbundanceMat[iChY_sub] =
        SpEOMatrixD::Zero(ImX->get_sizeU(), ImX->get_sizeV());
  }
  ...
  if (!fSetting.use_estimated_SRFs) {
    bool normalize_SRFs_to_fulfill_SumToOne_constraint =
        false;  // potentially beneficial when processing more realistic data
                // (not synthesized ones)
    read_SRF(&glPrms, &SRF, paths.fname_SRF, ',',
             normalize_SRFs_to_fulfill_SumToOne_constraint);
    MPI_Barrier(MPI_COMM_WORLD);
  } else {
    if (my_rank == 0) {
      cout << "#############################################################"
           ...
           << "##                                                         ##"
           << endl
           << "##   estimate spectral response functions and ImX shift    ##"
           << endl
           << "##                                                         ##"
           << endl
           << "#############################################################"
    ...
    SpEOMatrixD ImY_2D =
        SpEOMatrixD::Zero(ImY->get_NCh(), ImY->get_sizeU() * ImY->get_sizeV());
    SpEOMatrixD ImX_LR_2D = SpEOMatrixD::Zero(
        ImX_LR->get_NCh(), ImX_LR->get_sizeU() * ImX_LR->get_sizeV());

    transform_SpEODataset_to_2D(ImY, ImY_2D);
    transform_SpEODataset_to_2D(ImX_LR, ImX_LR_2D);
    if (my_rank == 0)
      cout << "estimate SFRs and ImX shift (band-wise) simultaneously via "
              "non-negative Least Squares.."
           << endl;
    bool est_successful =
        estimate_SRFs_and_ImX_shift(SRF, ImX_shift, ImY_2D, ImX_LR_2D, my_rank);
    if (my_rank == 0)
      cout << ".. done! The estimation completed with status: successful="
           << est_successful << endl
           << endl;
  ...
  int numProcBusy =
      pSetting.numProcPerPatch * min(glPrms.numPatchGroups, glPrms.NP_sub);
#endif
  ...
  if (my_rank < numProcBusy) {
    ...
    if (my_rank == 0) {
      ...
      if (fSetting.evaluate && fSetting.evaluate_ImZ_init) {
        ...
        // possibly correct Z_ref and ImY to selected spectral and spatial
        // subset
        ...
        // do the assessment
        PanSharp_Assessment_Eval(ImZ_ref, ImZ_init, ImY, &assMetrics_HR,
                                 &fSetting, &glPrms);
        // write assessment results in report
        report.addEvaluation(&assMetrics_HR, &dSetting, &fSetting, &oSetting,
                             &glPrms, HR_ASSESSMENT);
        // save assessment results
        int iterMain = 0;
        bool init_image_eval = true;
        save_evalResults(&paths, &glPrms, &fSetting, &assMetrics_HR, iterMain,
                         fSetting.iterMain, true,
                         fSetting.doFullImOptWithoutPatRec, init_image_eval,
                         final_evaluation);
        delete ImZ_ref;
    ...
    if (fSetting.doFullImOptWithoutPatRec) {
      ...
      for (iChZ = 0; iChZ < ImZ->get_NCh(); iChZ++) {
        ImZ->get_rasterBands()[iChZ]->bandDataMatD =
            ImZ_init->get_rasterBands()[iChZ]->bandDataMatD;
      }
    }
    ...
    for (iterMain = 0; iterMain < fSetting.iterMain; iterMain++) {
      ...
      if (!fSetting.doFullImOptWithoutPatRec) {
        JSparseFIHM_alg(SubspaceTransformMat, AbundanceMat, iterMain,
                        fSetting.iterMain, &paths, &dSetting, &fSetting,
                        &oSetting, &sSetting, &pSetting, &glPrms, ImX, ImX_LR,
                        ImY, ImZ, &SRF, comm_busy, group_busy, ImZ_init,
                        report);
        if (my_rank == 0) {
          //==================================================================//
          ...
          if (fSetting.evaluate) {
            if (iterMain == fSetting.iterMain - 1 && !fSetting.LQ_post_opt_im &&
                !(fSetting.set_neg_to_0 == 0 ||
                  (fSetting.set_neg_to_0 == 1 &&
                   fSetting.doFullImOptWithoutPatRec) ||
            ...
            // possibly correct Z_ref and ImY to selected spectral and spatial
            // subset
            cutRelevantInput(HR, imFlag_Z_ref, ImZ_ref, &dSetting, &glPrms,
                             false);
            cutRelevantInput(LR, imFlag_Y, ImY, &dSetting, &glPrms, false);
            SpEOAssessmentMetrics assMetrics_HR;
            // do the assessment
            PanSharp_Assessment_Eval(ImZ_ref, ImZ, ImY, &assMetrics_HR,
                                     &fSetting, &glPrms);
            // write assessment results in report
            report.addEvaluation(&assMetrics_HR, &dSetting, &fSetting,
                                 &oSetting, &glPrms, HR_ASSESSMENT);
            // save assessment results
            bool init_image_eval = false;
            save_evalResults(&paths, &glPrms, &fSetting, &assMetrics_HR,
                             iterMain, fSetting.iterMain, true,
                             fSetting.doFullImOptWithoutPatRec, init_image_eval,
                             final_evaluation);
            delete ImZ_ref;
            delete[] assMetrics_HR.RMSE_sep;
      ...
      if (fSetting.LQ_post_opt_im) {
        full_im_optimization_LS(report, SubspaceTransformMat, AbundanceMat,
                                &glPrms, &fSetting, &sSetting, &dSetting, ImX,
                                ImY, ImZ, &SRF, comm_busy, group_busy);
        for (int iChZ = 0; iChZ < glPrms.NChZ; iChZ++) {
          int datasize = ImZ->rasterBands[iChZ]->bandDataMatD.rows() *
                         ImZ->rasterBands[iChZ]->bandDataMatD.cols();
          MPI_Bcast((ImZ->rasterBands[iChZ]->bandDataMatD.data()), datasize,
                    MPI_DOUBLE, 0, comm_busy);  // MPI_COMM_WORLD);
        }
        if (my_rank == 0) {
          //==================================================================//
          //                   Assess reconstruction results                  //
          ...
          if (fSetting.evaluate) {
            ...
            // save assessment results
            bool init_image_eval = true;
            if (fSetting.evaluate_ImZ_init ||
                !fSetting.doFullImOptWithoutPatRec) {
              init_image_eval = false;
            }
            save_evalResults(&paths, &glPrms, &fSetting, &assMetrics_HR,
                             iterMain, fSetting.iterMain, false,
                             fSetting.doFullImOptWithoutPatRec, init_image_eval,
                             final_evaluation);
      ...
      if (oSetting.writeImageFile) {
        ...
        if (pSetting.store_patches_tmp_on_drive) {
          if (iterMain == 0) {
            paths.fname_ImZ_out =
                paths.fname_ImZ_out + "_iter" + (char)iterMain;
          }
          if (my_rank < numProcWrite) {
            ImZ->dataWriteParMPIIO(&report, &dSetting, &fSetting, &pSetting,
                                   &glPrms, &paths, comm_write);
          }
          if (my_rank == 0) {
            ImZ->writeENVIHeader(&report, &dSetting, &fSetting, &glPrms,
                                 &paths);
          }
        }
      }
      ...
      if (my_rank == 0) {
        ...
        //==================================================================//
        if (iterMain == 0) {
          save_fusion_setup(&paths, &dSetting, &fSetting, &pSetting, &sSetting,
                            &oSetting, &glPrms);
        }
      }
      if (iterMain == fSetting.iterMain - 1 &&
        ...
        for (int iChY = 0; iChY < ImZ->get_NCh(); iChY++) {
          ImZ->rasterBands[iChY]->bandDataMatD =
              ((ImZ->rasterBands[iChY]->bandDataMatD).array() > 0)
                  .select(ImZ->rasterBands[iChY]->bandDataMatD, 0);
        }
        //==================================================================//
        //                   Assess reconstruction results                  //
        //==================================================================//
        ...
        SpEOAssessmentMetrics assMetrics_HR;
        // do the assessment
        PanSharp_Assessment_Eval(ImZ_ref, ImZ, ImY, &assMetrics_HR, &fSetting,
                                 &glPrms);
        // write assessment results in report
        report.addEvaluation(&assMetrics_HR, &dSetting, &fSetting, &oSetting,
                             &glPrms, HR_ASSESSMENT);
      ...
      if (oSetting.writeImageFile) {
        if (pSetting.store_patches_tmp_on_drive) {
          ...
          if (iterMain == fSetting.iterMain - 1 ||
              oSetting.writeImageFileAfterEveryIter) {
            ...
            if (oSetting.writeImageFileAfterEveryIter) {
              ...
              fname_ImZ_out_tmp << paths.fname_ImZ_out << "_iter" << iterMain
                                << ".dat";
              paths.fname_ImZ_out = fname_ImZ_out_tmp.str();
            }
            if (my_rank < numProcWrite) {
              ImZ->dataWrite(&report, &dSetting, &fSetting, &pSetting, &glPrms,
                             &paths, comm_write, ImZ);
            }
            if (my_rank == 0) {
              ImZ->writeENVIHeader(&report, &dSetting, &fSetting, &glPrms,
                                   &paths);
      ...
      if (iterMain < fSetting.iterMain - 1) {
        if (!fSetting.doFullImOptWithoutPatRec) {
          if (my_rank == 0) {
            cout << "Copy the reconstruction results, ImZ, to ImZ_init for "
                    "next iteration.."
                 << endl;
          }
          for (int iChY = 0; iChY < glPrms.NChY; iChY++) {
            ImZ_init->get_rasterBands()[iChY]->bandDataMatD =
                ImZ->get_rasterBands()[iChY]->bandDataMatD;
            ImZ->get_rasterBands()[iChY]->bandDataMatD = SpEOMatrixD::Zero(
                ImZ->get_rasterBands()[iChY]->bandDataMatD.rows(),
                ImZ->get_rasterBands()[iChY]->bandDataMatD.cols());
          }
        }
      }
    }
