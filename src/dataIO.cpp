...
void printOneAsTwoDimArray(int nVSz, int nUSz, float *oneDimArray, int numRows,
  ...
  cout << "\tiV \t 0 \t 1 \t 2 \t  ...  \t";
  for (int col = 0; col < numCols; col++) cout << " " << cols[col] << " \t";
  cout << "...  \t" << nVSz - 3 << "\t" << nVSz - 2 << "\t" << nVSz - 1 << endl;
  cout << "iU      "
          "--------------------------------------------------------------------"
          "----- "
       << endl;
...
void SpEODataset::dataRead(string fnm, SpEOReport *report) {
    ...
    switch (this->get_imFlag()) {
      ...
      case imFlag_X_sim:
        report->file << "ImX_sim             *"
                     << "\n";
        break;
      case imFlag_X_sim_LR:
        report->file << "ImX_sim_LR          *"
                     << "\n";
        break;
      case imFlag_Y:
        report->file << "ImY                 *"
      ...
      case imFlag_Z_ref:
        ...
        break;
      case imFlag_Z_init:
        report->file << "ImZ_init            *"
                     << "\n";
        break;
      default:
        report->file << "\n\n ERROR: program terminated unsuccessfully due to "
                        "try to read an image with an UNKNOWN IMAGE FLAG!"
                     << "\n";
        cerr << "\n\n ERROR: program terminated unsuccessfully due to try to "
                "read an image with an UNKNOWN IMAGE FLAG!"
             << "\n";
        exit(2);
        break;
    }
  ...
  if (my_rank == 0) {
    report->file << " - Information about the (GDAL-)dataset:\n"
                 ...
                 << this->imFormatLong << "\n"
                 << "     - Image Size:\t" << this->sizeV << "(VSize) x "
                 << this->sizeU << "(USize) x " << this->NCh
                 << "(RasterCount)\n";
    ...
    if (poDataset->GetGeoTransform(this->geoTransform) == CE_None) {
      report->file << "     - Origin =\t(top left x, top left y) = ("
                   ...
                   << ")\n"
                   << "     - Pixel Size =\t(w-e pixel resolution, n-s pixel "
                      "resolution) = ("
  ...
  for (int iCh = 1; iCh < this->NCh + 1; iCh++) {
    ...
    if (!(bGotMin && bGotMax)) {
      GDALComputeRasterMinMax(poBand, TRUE,
                              this->rasterBands[iCh - 1]->minMaxVal);
    }
    this->rasterBands[iCh - 1]->dataType =
     ...
     * GDALRasterBand::RasterIO() method. This method will automatically take
     * care of data type conversion, up/down sampling and windowing. The
     * following code will read the first scanline of data into a similarly
     * sized buffer, converting it to floating point as part of the operation.
     */
    // Eigen Matrix, which makes data handling easier. Perhaps, the band data
    // array won't be needed in the future.
    if (this->get_imFlag() == imFlag_Z || this->get_imFlag() == imFlag_Z_init) {
      double *bandData =
          (double *)CPLMalloc(sizeof(double) * this->sizeV * this->sizeU);
      poBand->RasterIO(GF_Read, 0, 0, this->sizeV, this->sizeU, bandData,
                       this->sizeV, this->sizeU, GDT_Float64, 0, 0);
      this->rasterBands[iCh - 1]->bandDataMatD =
          SpEOMatrixD::Zero(this->sizeU, this->sizeV);
      this->rasterBands[iCh - 1]->bandDataMatD =
          SpEOMatrixD::Map(bandData, this->sizeU, this->sizeV);
      CPLFree(bandData);
      bandData = NULL;
      if (contains_inf_or_nan(this->rasterBands[iCh - 1]->bandDataMatD)) {
        cerr << endl
             ...
             << endl
             << " E" << endl
             << " E    NaN or INF   found in     this->rasterBands[iCh-1="
             << iCh - 1 << "]->bandDataMatD" << endl
             << " E" << endl
             << " EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"
             << endl
             << endl
      ...
      float *bandData =
          (float *)CPLMalloc(sizeof(float) * this->sizeV * this->sizeU);
      poBand->RasterIO(GF_Read, 0, 0, this->sizeV, this->sizeU, bandData,
                       this->sizeV, this->sizeU, GDT_Float32, 0, 0);
      this->rasterBands[iCh - 1]->bandDataMat =
          SpEOMatrixF::Zero(this->sizeU, this->sizeV);
      this->rasterBands[iCh - 1]->bandDataMat =
          SpEOMatrixF::Map(bandData, this->sizeU, this->sizeV);
      CPLFree(bandData);
      bandData = NULL;
...
void SpEODataset::dataWrite(SpEOReport *report, SpEODataIOSetting *dSet,
                            SpEOFusionSetting *fSet, SpEOParallelSetting *pSet,
                            SpEOGlobalParams *glPrms, SpEOPaths *paths,
                            MPI_Comm comm_write, SpEODataset *ImZ) {
  int my_rank;
  int my_processes;
  ...
  if (dSet->saveAsDouble) {
    ...
    ftype = MPI_DOUBLE;
    buftype = MPI_DOUBLE;
    // communicator group comm_write opens the data file for writing only (and
    // creating, if necessary) */
    result =
        MPI_File_open(comm_write, (char *)paths->fname_ImZ_out.c_str(),
                      MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &fh);
    if (result != MPI_SUCCESS) {
      sample_error(result, (char *)"MPI_File_open");
    }
    disp = 0;
    result = MPI_File_set_view(fh, disp, etype, ftype, (char *)"native",
                               MPI_INFO_NULL);
    if (result != MPI_SUCCESS) {
    ...
    while (iL < nL) {  // as long as there are LR pixels to work on
      ...
      if (uL >= dSet->uLFirst && vL >= dSet->vLFirst && uL <= dSet->uLLast &&
          vL <= dSet->vLLast) {
        // initialization
        for (iChZ = 0; iChZ < this->NCh; iChZ++) {
          area_sum[iChZ] = ImZ->get_rasterBands()[iChZ]->bandDataMatD.block(
              uH_sub, vH_sub, fDS, fDS);
        }
        ...
        if (dSet->uLFirst == 0 && dSet->uLLast == glPrms->sizeUL - 1 &&
          ...
          for (u = 0; u < fDS; u++) {  // all HR lines/rows in the current area
            for (v = 0; v < fDS;
                 v++) {  // all HR samples/columns in the current area
              // Allocate and initialize a buffer (buf_spectrum_WR) containing
              // this->NCh unsigned shorts, where the unsigned short at location
              // iChZ will set to (unsigned int)area_sum[iChZ](u,v).
              for (iChZ = 0; iChZ < this->NCh; iChZ++) {
                buf_spectrum_WR[iChZ] = (double)area_sum[iChZ](u, v);
              }
              // specify the offset relative to the displacement position. In
              // compare to displacement, the offset value is given NOT IN
              // BYTES, but IN MULTIPLES OF etype (here
              // etype==MPI_UNSIGNED_SHORT)
              offset = this->NCh * ((uL * this->sizeV * fDS) + this->sizeV * u +
                                    (vL * fDS) + v);

              // Write the buffer (buf_spectrum_WR)

              result = MPI_File_write_at(fh, offset, buf_spectrum_WR, this->NCh,
                                         buftype, &status);
              if (result != MPI_SUCCESS) {
          ...
          for (u = 0; u < fDS; u++) {  // all HR lines/rows in the current area
            for (v = 0; v < fDS;
              ...
              offset =
                  this->NCh *
                  ((((uL - dSet->uLFirst) * fDS + u) * glPrms->sizeVH_red) +
                   (vL - dSet->vLFirst) * fDS + v);
              // Write the buffer (buf_spectrum_WR)
              result = MPI_File_write_at(fh, offset, buf_spectrum_WR, this->NCh,
                                         buftype, &status);
              if (result != MPI_SUCCESS) {
    ...
    buf_spectrum_WR =
        (unsigned short *)malloc(this->NCh * sizeof(unsigned short));
    MPI_File fh;
    MPI_Offset disp, offset;
    MPI_Datatype etype, ftype, buftype;
    MPI_Status status;
...
void SpEODataset::writeENVIHeader(SpEOReport *report, SpEODataIOSetting *dSet,
...
void SpEODataset::dataWriteParMPIIO(SpEOReport *report, SpEODataIOSetting *dSet,
                                    SpEOFusionSetting *fSet,
  ...
  int u, v, iL, uL, vL, uH, vH, nL, pszL, pszH, a, uP, vP, iP, pCnt, fDS, iChZ,
      uPH, vPH;
  fDS = glPrms->fDS;
  nL = glPrms->sizeUL * glPrms->sizeVL;
  pszL = fSet->patchsize;
  ...
  if (my_rank == 0) {
    cout << "\n"
         ...
         << "##                                                                "
            "    ##"
         << "\n"
         << "##    Write image ImZ (from tmp patches on drive) in parallel "
            "using.. ##"
         << "\n"
         << "##      (1) MPI I/O                                               "
            "    ##"
    ...
    report->file << "\n"
                 ...
                 << "##                                                        "
                    "            ##"
                 << "\n"
                 << "##    Write image ImZ (from tmp patches on drive) in "
                    "parallel using.. ##"
                 << "\n"
                 << "##      (1) MPI I/O                                       "
                    "            ##"
  ...
  // communicator group comm_write opens the data file for writing only (and
  // creating, if necessary) */
  result = MPI_File_open(comm_write, (char *)paths->fname_ImZ_out.c_str(),
                         MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &fh);
  if (result != MPI_SUCCESS) {
    sample_error(result, (char *)"MPI_File_open");
  }
  ...
  while (iL < nL) {  // as long as there are LR pixels to work on
    ...
    if (uL >= dSet->uLFirst && vL >= dSet->vLFirst && uL <= dSet->uLLast &&
      ...
      // first patch that influences the current part (LR pixel) of the image
      // that is to be written
      pCnt = 0;
      uP = ceil(max(0.0, ((double)(uL - pszL + 1))) / a);
      vP = ceil(max(0.0, ((double)(vL - pszL + 1))) / a);
      if (uP < glPrms->uPFirst) {
        uP = glPrms->uPFirst;
      }
      ...
      while (uP * a <= uL && uP * a + pszL - 1 < glPrms->sizeUL) {
        // calc relative (local) position of area (part of patch) w.r.t. to the
        // patch's corner coordinates (0,0)
        uPH = uH - uP * a * fDS;
        vP = ceil(max(0.0, ((double)(vL - pszL + 1))) / a);
        if (vP < glPrms->vPFirst) {
          vP = glPrms->vPFirst;
        ...
        while (vP * a <= vL && vP * a + pszL - 1 < glPrms->sizeVL) {
          vPH = vH - vP * a * fDS;
          SpEODataset::read_patch(paths, iL, iP, uP, pCnt, vP, pszH, uPH, vPH,
                                  fDS, myReadMat, patch, pVecHR, area_sum, dSet,
                                  my_rank);
        }
      ...
      if (dSet->uLFirst == 0 && dSet->uLLast == glPrms->sizeUL - 1 &&
        ...
        for (u = 0; u < fDS; u++) {  // all HR lines/rows in the current area
          for (v = 0; v < fDS;
            ...
            // specify the offset relative to the displacement position. In
            // compare to displacement, the offset value is given NOT IN BYTES,
            // but IN MULTIPLES OF etype (here etype==MPI_UNSIGNED_SHORT)
            offset = this->NCh * ((uL * this->sizeV * fDS) + this->sizeV * u +
                                  (vL * fDS) + v);
...
void SpEODataset::read_patch(SpEOPaths *paths, int iL, int &iP, int uP,
                             int &pCnt, int &vP, int pszH, int uPH, int vPH,
                             int fDS, SpEOMatrixF &myReadMat,
                             SpEOMatrixF &patch, SpEOVectorF &pVecHR,
                             SpEOMatrixF *area_sum, SpEODataIOSetting *dSet,
                             int my_rank) {
  // read patch
  ...
  if (stat_CSV_read == -1) {
    cout << "[" << my_rank << "] ERROR while writing pixel iL=" << iL
         << ": The .csv file '" << buf
         << "' does not exist in any of the specified TMP patch directories!"
         << endl;
  } else {
...
void setMetaInfo(SpEODataset *ImZ, SpEODataset *ImY, SpEODataset *ImX,
  ...
  for (int iChZ = 0; iChZ < ImZ->NCh; iChZ++) {
    ...
    ImZ->rasterBands[iChZ]->bandDataMatD =
        SpEOMatrixD::Zero(ImZ->sizeU, ImZ->sizeV);
...
void prepGroupCalcForJSpFI(
    int firstBandY, int lastBandY, int panBand, SpEOGlobalParams &glPrms,
    SpEOFusionSetting &fSetting, SpEODataIOSetting &dSetting,
    SpEOMatrixD &SRF_orig, SpEOMatrixD &SRF, SpEOMatrixD &filter_coeff,
    SpEOMatrixD &gauss_filter, SpEODataset *ImX, SpEODataset *ImX_LR,
    SpEODataset *ImY, SpEODataset *ImZ, SpEODataset *ImX_tmp,
    SpEODataset *ImX_LR_tmp, SpEODataset *ImY_tmp, SpEODataset *ImZ_tmp) {
  int my_rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
  ...
  if (firstBandY < 0 || lastBandY >= glPrms.NChY) {
    if (my_rank == 0) {
      cerr << endl
           << "ERROR: The condition (firstBandY=>0 && lastBandY<glPrms.NChY) "
              "is NOT filfilled!: firstBandY=="
           << firstBandY << ", lastBandY==" << lastBandY
           << ", and glPrms.NChY==" << glPrms.NChY << endl
           << endl;
    }
   ...
   ********************************/
  // initialize ImX_tmp and ImX_LR_tmp (including meta data) from ImX and
  // ImX_LR, respectively
  firstBand = 0;
  lastBand = 0;
  ...
  if (panBand == -1) {
    ...
    ImX_tmp->get_rasterBands()[0]->bandDataMat =
        ImZ->get_rasterBands()[panBand]->bandDataMatD.cast<float>();
    /******************
     * low-pass filter and down-sample
     ******************///
    SpEOMatrixD ImX_tmpD_F = ImZ->get_rasterBands()[panBand]->bandDataMatD;
    SpEOMatrixD ImX_LR_tmpD_F;
    fast_filter(ImX_LR_tmpD_F, ImX_tmpD_F, gauss_filter, filter_coeff,
                glPrms.fDS);  // fast method (intuitive)
...
void SpEODataset::copy_from_dataset(int firstBand, int lastBand,
                                    SpEODataset *sourceIm) {
  ...
  for (int iCh = 0; iCh < this->NCh; iCh++) {
    ...
    this->rasterBands[iCh]->bandDataType =
        sourceIm->get_rasterBands()[firstBand + iCh]->get_bandDataType();
    this->rasterBands[iCh]->nBand = iCh + 1;
    this->rasterBands[iCh]->minMaxVal[0] =
        sourceIm->get_rasterBands()[firstBand + iCh]->get_minMaxVal()[0];
    this->rasterBands[iCh]->minMaxVal[1] =
        sourceIm->get_rasterBands()[firstBand + iCh]->get_minMaxVal()[1];
    this->rasterBands[iCh]->dataType =
        sourceIm->get_rasterBands()[firstBand + iCh]->get_dataType();
    this->rasterBands[iCh]->colorInterp =
        sourceIm->get_rasterBands()[firstBand + iCh]->get_colorInterp();
    if (sourceIm->get_imFlag() == imFlag_Z ||
        sourceIm->get_imFlag() == imFlag_Z_LR) {
      this->rasterBands[iCh]->bandDataMatD =
          SpEOMatrixD::Map(sourceIm->get_rasterBands()[firstBand + iCh]
                               ->get_bandDataMatD()
                               ->data(),
                           sourceIm->get_rasterBands()[firstBand + iCh]
                               ->get_bandDataMatD()
                               ->rows(),
                           sourceIm->get_rasterBands()[firstBand + iCh]
                               ->get_bandDataMatD()
      ...
      this->rasterBands[iCh]->bandDataMat =
          SpEOMatrixF::Map(sourceIm->get_rasterBands()[firstBand + iCh]
                               ->get_bandDataMat()
                               ->data(),
                           sourceIm->get_rasterBands()[firstBand + iCh]
                               ->get_bandDataMat()
...
void SpEODataset::copyMetaInfoFromDatasets(SpEODataset *sourceImSpatial,
  ...
  for (int iCh = 0; iCh < this->NCh; iCh++) {
    ...
    this->rasterBands[iCh]->bandDataType =
        sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
            ->get_bandDataType();
    this->rasterBands[iCh]->nBand = iCh + 1;
    this->rasterBands[iCh]->minMaxVal[0] =
        sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
    ...
    this->rasterBands[iCh]->minMaxVal[1] =
        sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
            ->get_minMaxVal()[1];
    this->rasterBands[iCh]->dataType =
        sourceImSpatial->get_rasterBands()[iChSoureImSpatial]->get_dataType();
    this->rasterBands[iCh]->colorInterp =
        sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
            ->get_colorInterp();

    if (dataFormat == SpEODouble) {
      this->rasterBands[iCh]->bandDataMatD = SpEOMatrixD::Zero(
          sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
              ->get_bandDataMatD()
              ->rows(),
          sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
      ...
      this->rasterBands[iCh]->bandDataMat = SpEOMatrixF::Zero(
          sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
              ->get_bandDataMatD()
              ->rows(),
          sourceImSpatial->get_rasterBands()[iChSoureImSpatial]
...
void SpEODataset::fill_band_data(SpEODataset *sourceIm, int firstBand,
                                 int lastBand) {
  int my_rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
  ...
  if (lastBand - firstBand + 1 != sourceIm->NCh) {
    if (my_rank == 0) {
      cerr << endl
           << "ERROR: in SpEODataset::fill_band_data(..): The number of bands "
              "of the source image has to be equal to lastBand-firstBand+1"
           << endl
           << endl;
    }
    MPI_Barrier(MPI_COMM_WORLD);
  ...
  for (int iCh = firstBand; iCh <= lastBand; iCh++) {
    ...
    this->rasterBands[iCh]->bandDataType =
        sourceIm->get_rasterBands()[iCh - firstBand]->get_bandDataType();
    this->rasterBands[iCh]->nBand = iCh + 1;
    this->rasterBands[iCh]->minMaxVal[0] =
        sourceIm->get_rasterBands()[iCh - firstBand]->get_minMaxVal()[0];
    this->rasterBands[iCh]->minMaxVal[1] =
        sourceIm->get_rasterBands()[iCh - firstBand]->get_minMaxVal()[1];
    this->rasterBands[iCh]->dataType =
        sourceIm->get_rasterBands()[iCh - firstBand]->get_dataType();
    this->rasterBands[iCh]->colorInterp =
        sourceIm->get_rasterBands()[iCh - firstBand]->get_colorInterp();
    if (sourceIm->get_imFlag() == imFlag_Z ||
        sourceIm->get_imFlag() == imFlag_Z_LR) {
      this->rasterBands[iCh]->bandDataMatD =
          SpEOMatrixD::Map(sourceIm->get_rasterBands()[iCh - firstBand]
                               ->get_bandDataMatD()
                               ->data(),
                           sourceIm->get_rasterBands()[iCh - firstBand]
                               ->get_bandDataMatD()
                               ->rows(),
                           sourceIm->get_rasterBands()[iCh - firstBand]
                               ->get_bandDataMatD()
      ...
      this->rasterBands[iCh]->bandDataMat =
          SpEOMatrixF::Map(sourceIm->get_rasterBands()[iCh - firstBand]
                               ->get_bandDataMat()
                               ->data(),
                           sourceIm->get_rasterBands()[iCh - firstBand]
                               ->get_bandDataMat()
...
void cutRelevantInput(SpEODataset *ImX, SpEODataset *ImY,
  ...
  if (dSet->chBundleFirst < 0) {
    if (my_rank == 0) {
      cout << "WARNING: chBundleFirst must not be negative! It got corrected "
              "from "
           << dSet->chBundleFirst << " to 0." << endl;
    }
    dSet->chBundleFirst = 0;
  }
  if (dSet->chBundleFirst >= ImY->NCh) {
    if (my_rank == 0) {
      cout << "chBundleFirst has been corrected from " << dSet->chBundleFirst
           << " to " << ImY->NCh - 1 << "=NChY-1," << endl;
    }
    dSet->chBundleFirst = ImY->NCh - 1;
  }
  if (dSet->chBundleLast < 0) {
    if (my_rank == 0) {
      cout << "WARNING: chBundleLast must not be negative! It got corrected "
              "from "
           << dSet->chBundleLast << " to 0." << endl;
    }
    dSet->chBundleLast = 0;
  }
  if (dSet->chBundleLast >= ImY->NCh) {
    if (my_rank == 0) {
      cout << "chBundleLast has been corrected from " << dSet->chBundleLast
           << " to " << ImY->NCh - 1 << "=NChY-1." << endl;
    }
  ...
  if (dSet->chBundleLast - dSet->chBundleFirst + 1 < ImY->NCh) {
    if (my_rank == 0) {
      cout << "NChY has been corrected from " << ImY->NCh << " to "
           << dSet->chBundleLast - dSet->chBundleFirst + 1
           << "=chBundleLast-chBundleFirst+1." << endl;
    }
...
void cutRelevantInput(SpEOResolution eRes, SpEOImFlag imFlag, SpEODataset *Im,
                      SpEODataIOSetting *dSet, SpEOGlobalParams *glPrms,
                      bool cutOffLRBoundaryPixels) {
  short NCh_buf = Im->NCh;
  if (!(imFlag == imFlag_X || imFlag == imFlag_X_LR || imFlag == imFlag_X_sim ||
        imFlag == imFlag_X_sim_LR)) {
  ...
  if (eRes == LR) {
    ...
    cout << "WARNING: First argument in the function cutRelevantInput has to "
            "specify the image resolution (set either to 'LR' of 'HR')! It got "
            "set to 'LR' by default!"
         << endl;
    mySizeU_red = glPrms->sizeUL_red;
    mySizeV_red = glPrms->sizeVL_red;
    myUFirst = dSet->uLFirst;
  ...
  if (!(dSet->uLFirst == 0 && dSet->uLLast == glPrms->sizeUL - 1 &&
    ...
    for (int iCh = 0; iCh < Im->NCh; iCh++) {
      SpEOMatrixF tmp = Im->get_rasterBands()[iCh]->bandDataMat.block(
          myUFirst, myVFirst, mySizeU_red, mySizeV_red);
...
void SpEOReport::initialize(SpEOPaths *paths, SpEODataIOSetting *dSetting,
                            SpEOFusionSetting *fSetting,
                            SpEOOutputSetting *oSetting,
                            SpEOSolverSetting *sSetting,
                            SpEOParallelSetting *pSetting, int argc,
                            char **argv) {
  ...
  string datePretty = "20" + this->curTime.substr(0, 2) + "-" +
                      this->curTime.substr(2, 2) + "-" +
                      this->curTime.substr(4, 2);
  string timePretty = this->curTime.substr(7, 2) + ":" +
                      this->curTime.substr(9, 2) + ":" +
                      this->curTime.substr(11, 2);
  ...
  this->file << "##############################################################"
             ...
             << "###                               J-SparseFI-HM               "
                "              ###"
             << "\n"
             << "###   [ Jointly Sparse Fusion of Hyperspectral and "
  ...
  this->file << " - directory where tmp patches were/are stored : "
  ...
  this->file << "\n"
             ...
             << "*========================================*"
             << "\n"
             << " - first Y band to be processed (before possible correction): "
             << dSetting->chBundleFirst << "\n"
             << " - last Y band to be processed (before possible correction): "
             << dSetting->chBundleLast << "\n"
             << " - uLFirst: first vertical low resolution pixel to be "
                "processed (before possible correction):   "
             << dSetting->uLFirst << "\n"
             << " - uLLast:  last vertical low resolution pixel to be "
                "processed (before possible correction):    "
             << dSetting->uLLast << "\n"
             << " - vLFirst: first horizontal low resolution pixel to be "
                "processed (before possible correction): "
             << dSetting->vLFirst << "\n"
             << " - vLLast:  last horizontal low resolution pixel to be "
                "processed (before possible correction):  "
             << dSetting->vLLast << "\n";
  this->file << "\n"
             << "*========================================*"
  ...
  this->file << " - two-step estimation:                     "
             << fSetting->two_step_estimation << "\n";
  ...
  this->file << "     -> Patch-wise coefficient estimation parameters:\n"
             << "                       - lambda_X_ABC:  "
             << fSetting->lambdaX_ABC << "\n"
             << "                       - lambda_Y_ABC:  "
             << fSetting->lambdaY_ABC << "\n"
             << "                       - lambda_Z_ABC:  "
             << fSetting->lambdaZ_ABC << "\n"
             << "        - lambdaZ_ABC_in_1st_iter:      "
             << fSetting->lambdaZ_ABC_in_1st_iter << "\n"
             << "     -> full image CGLS settings: ACTIVE="
             << fSetting->LQ_post_opt_im << "\n"
             << "                       - lambda_X_im:      "
             << fSetting->lambdaX_im << "\n"
             << "                       - lambda_Y_im:      "
             << fSetting->lambdaY_im << "\n"
             << "                       - maxiter CGLS_im:  "
             << sSetting->maxiter_CGLS_im << "\n"
             << "                       - tol r CGLS_im:    "
             << sSetting->tol_r_CGLS_im << "\n"
             << "                       - subspace_transformation_type:  "
             << fSetting->subspace_transform_type << "\n"
             << "                       - subspace_dim:                  "
             << fSetting->subspace_dim << "\n";

  this->file
      << " - image normalization:                     " << fSetting->nrmlIm
      << "\n"
      << " - dictionary normalization:                " << fSetting->nrmlDicts
      << "\n"
      << " - mean subtraction:                        " << fSetting->substrMean
      << "\n"
      << " - patch size (LR, one side):               " << fSetting->patchsize
      << "\n"
      << " - overlap (LR, one side):                  " << fSetting->overlap
      << "\n"
      << " - lambda:                                  " << fSetting->lambda
      << "\n"
      << " - NDP (before possible correction):        " << fSetting->NDP << "\n"
      ...
      << fSetting->theta << "\n"
      << " - tol_SRF (tolerance for decision matrix): " << fSetting->tol_SRF
      << "\n"
      << " - dictionary selection method ID:          " << fSetting->dictselect
      << " (";
  switch (fSetting->dictselect) {
    case 0:
      this->file << "Dictionary contains ONLY the current patch -> (NDP=1) & "
                    "Alpha is calculated by least squares";
      break;
    case 1:
      this->file << "Nearest Neighbors";
      break;
    case 2:
      this->file << "PanLR norm";
      break;
    case 3:
      this->file << "SRF approximate PanLR norm";
      break;
    case 4:
      this->file << "PanHR norm (POSITIVE PanHR correlation)";
      break;
    case 5:
      this->file << "PanLR-PanHR joint ranking";
      break;
    case 6:
      this->file << "ABSOLUTE PanHR correlation";
      break;
    case 7:
      this->file
          << "PanHR anti-correlation, including current patch as first atom";
      break;
    case 8:
      this->file << "Random, including current patch as first atom";
      break;
    case 9:
      this->file << "PanHR self uncorrelated basis approximation, including "
                    "current patch as first atom";
      break;
    default:
      this->file << "\n\n ERROR: program terminated unsuccessfully due an "
                    "UNKNOWN DICTIONARY SELECTION FLAG!"
                 << "\n";
      cerr << "\n\n ERROR: program terminated unsuccessfully due an UNKNOWN "
              "DICTIONARY SELECTION FLAG!"
           << "\n";
      exit(2);
      break;
  }
  this->file
      ...
      << pSetting->numProcGrp << "\n"
      << " - number of processes per patch (before possible correction):  "
      << pSetting->numProcPerPatch << "\n"
      << " - total number of processes:                                   "
      << my_processes << "\n"
      << " - work stealing turns (parallelization strategy):              "
      << pSetting->workStealingTurns << "\n"
      << " - store rec. patches temporarily on hard drive?                "
      << pSetting->store_patches_tmp_on_drive << "\n"
      << " - max number of processors used for writing final image        "
      << pSetting->parWrNumProc << "\n"
      << "\n"
      << "*========================================*"
      << "\n"
      ...
      << "\n"
      << " - write image file?:                               "
      << oSetting->writeImageFile << "\n"
      << " - save coefficient vectors?:                       "
      << oSetting->saveAlphas << "\n"
      << " - if so, which coefficient vectors?: first patch:  "
      << oSetting->pFirstAlpha << "\n"
      << " - if so, which coefficient vectors?: last patch:   "
      << oSetting->pLastAlpha << "\n"
      << " - save local dictionary coordinates?:              "
      << oSetting->saveDicts << "\n"
      << " - if so, for which patches?: first patch:          "
      << oSetting->pFirstDict << "\n"
      << " - if so, for which patches?: last patch:           "
      << oSetting->pLastDict << "\n"
      << " - evaluation precision (number of digits):         "
      << oSetting->prec << "\n"
      << "\n";
  ...
  cout << "\n"
       ...
       << "#                               J-SparseFI-HM                       "
          "     #\n"
       << "#   [ Jointly Sparse Fusion of Hyperspectral and Multispectral "
          "Images ]  #\n"
       << "#                                                                   "
          "     #\n"
       << "#==================================================================="
       ...
       << " - last Y band to be processed (before possible correction): "
       << dSetting->chBundleLast << "\n"
       << " - uLFirst: first vertical low resolution pixel to be processed "
          "(before possible correction):   "
       << dSetting->uLFirst << "\n"
       << " - uLLast:  last vertical low resolution pixel to be processed "
          "(before possible correction):    "
       << dSetting->uLLast << "\n"
       << " - vLFirst: first horizontal low resolution pixel to be processed "
          "(before possible correction): "
       << dSetting->vLFirst << "\n"
       << " - vLLast:  last horizontal low resolution pixel to be processed "
          "(before possible correction):  "
       << dSetting->vLLast << "\n"
       << "\n"
       << "*========================================*"
  ...
  cout << "     -> solver settings:  - maxiter_out:    "
       ...
       << " - Nc (number of bundled channels - before possible correction):    "
          "    "
       << fSetting->Nc << "\n"
       << " - No (number of overlapping channels - before possible "
          "correction):    "
       << fSetting->No << "\n"
       << " - Nc_max (maximum size of spectral group above groups will"
       << "\n"
       << "   be double-checked and perhaps split into subgoups (int)):        "
          "    "
       << fSetting->Nc_max << "\n"
       << " - theta (minimum cross-correlation within spectral goups "
          "(double)):   "
       << fSetting->theta << "\n"
       << " - tol_SRF (tolerance for decision matrix): " << fSetting->tol_SRF
       << "\n"
...
void SpEOReport::addEvaluation(SpEOAssessmentMetrics *assessm_res_HR,
                               ...
                               SpEOFusionSetting *fSetting,
                               SpEOOutputSetting *oSetting,
                               SpEOGlobalParams *glPrms, int type) {
  SpEOAssessmentMetricsStr *assessm_res_HR_Str = new SpEOAssessmentMetricsStr;
  SpEOAssessmentMetricsStr_Sep *assessm_res_HR_StrSep =
      new SpEOAssessmentMetricsStr_Sep[glPrms->NChY];

  int assessNum = NUM_OF_ASSESSMENT, temp;
  int allDigits[assessNum][glPrms->NChY + 1];
  int maxDigits[assessNum];

  assessm_res_HR_Str->RMSE_str =
      double2str(assessm_res_HR->RMSE_mean, oSetting->prec);
  assessm_res_HR_Str->PSNR_str =
      double2str(assessm_res_HR->PSNR_mean, oSetting->prec);
  assessm_res_HR_Str->CC_str =
      double2str(assessm_res_HR->CC_mean, oSetting->prec);
  assessm_res_HR_Str->ERGAS_str =
      double2str(assessm_res_HR->ERGAS_mean, oSetting->prec);
  assessm_res_HR_Str->UIQI_str =
      double2str(assessm_res_HR->UIQI_mean, oSetting->prec);
  assessm_res_HR_Str->DD_str =
      double2str(assessm_res_HR->DD_mean, oSetting->prec);
  assessm_res_HR_Str->SAM_str = double2str(assessm_res_HR->SAM, oSetting->prec);
  assessm_res_HR_Str->DLambda_str =
      double2str(assessm_res_HR->DLambda_mean, oSetting->prec);
  assessm_res_HR_Str->AG_orig_str =
      double2str(assessm_res_HR->AG_orig_mean, oSetting->prec);
  assessm_res_HR_Str->AG_rec_str =
      double2str(assessm_res_HR->AG_rec_mean, oSetting->prec);
  ...
  for (int i = 0; i < glPrms->NChY; i++) {
    assessm_res_HR_StrSep[i].RMSE_sep_str =
        double2str(assessm_res_HR->RMSE_sep[i], oSetting->prec);
    assessm_res_HR_StrSep[i].PSNR_sep_str =
        double2str(assessm_res_HR->PSNR_sep[i], oSetting->prec);
    assessm_res_HR_StrSep[i].CC_sep_str =
        double2str(assessm_res_HR->CC_sep[i], oSetting->prec);
    assessm_res_HR_StrSep[i].ERGAS_sep_str =
        double2str(assessm_res_HR->ERGAS_sep[i], oSetting->prec);
    assessm_res_HR_StrSep[i].UIQI_sep_str =
        double2str(assessm_res_HR->UIQI_sep[i], oSetting->prec);
    assessm_res_HR_StrSep[i].DD_sep_str =
        double2str(assessm_res_HR->DD_sep[i], oSetting->prec);
    assessm_res_HR_StrSep[i].AG_orig_sep_str =
        double2str(assessm_res_HR->AG_orig_sep[i], oSetting->prec);
    assessm_res_HR_StrSep[i].AG_rec_sep_str =
        double2str(assessm_res_HR->AG_rec_sep[i], oSetting->prec);
  ...
  for (int i = 0; i < assessNum; i++) {
    for (int j = 0; j <= glPrms->NChY; j++) {
      if (allDigits[i][j] > temp) temp = allDigits[i][j];
    }
    maxDigits[i] = temp;
    temp = 0;
  }
  createTable(assessm_res_HR_Str, assessm_res_HR_StrSep, dSetting, fSetting,
              glPrms, maxDigits, type, this);
...
void SpEOReport::addGlobalParams(SpEOGlobalParams *glPrms,
                                 SpEOFusionSetting *fSetting) {
  this->file.open(this->fileName.c_str(),
                  fstream::in | fstream::out | fstream::app);
  this->file
      ...
      << " - theta (minimum cross-correlation within spectral goups (double)): "
      << fSetting->theta << "\n"
      << " - NChX (number of bands in image ImX):            " << glPrms->NChX
      << "\n"
      << " - NChY (number of bands in image ImY):            " << glPrms->NChY
      << "\n"
      << " - NChZ (number of bands in image ImZ):            " << glPrms->NChZ
      << "\n"
      << " - fDS (down-sampling factor):                     " << glPrms->fDS
      << "\n"
      ...
      << " - NP_sub (total number of actually processed patches per band):     "
         "                "
      << glPrms->NP_sub << "\n"
      << " - NPU_sub (number of actually processed patches in horizontal "
         "direction per band):  "
      << glPrms->NPU_sub << "\n"
      << " - NPV_sub (number of actually processed patches in vertical "
         "direction per band):    "
      << glPrms->NPV_sub << "\n";
  this->file << "\n";
...
string createStr(int number, string Str) {
  string strArr;
  for (int i = 0; i < number; i++) strArr += Str;
  return strArr;
}
...
void createTable(SpEOAssessmentMetricsStr *assessmStr,
  ...
  if (maxDigits[0] < 4) maxDigits[0] = 4;  // because length("RMSE")     = 4
  if (maxDigits[1] < 2) maxDigits[1] = 2;  // because length("CC")       = 2
  if (maxDigits[2] < 5) maxDigits[2] = 5;  // because length("ERGAS")    = 5
  if (maxDigits[3] < 4) maxDigits[3] = 4;  // because length("UIQI")     = 4
  if (maxDigits[4] < 2) maxDigits[4] = 2;  // because length("DD")       = 2
  if (maxDigits[5] < 3) maxDigits[5] = 3;  // because length("SAM")      = 3
  if (maxDigits[6] < 8) maxDigits[6] = 8;  // because length("D_lambda") = 8
  if (maxDigits[7] < 7) maxDigits[7] = 7;  // because length("AG_orig")  = 7
  if (maxDigits[8] < 6) maxDigits[8] = 6;  // because length("AG_rec")   = 6
  if (maxDigits[9] < 4) maxDigits[9] = 4;  // because length("PSNR")     = 4
  ...
  cout << lineBeg << createStr(firstColLen + 2, midLinSym) << delimiter2
       << createStr(totalElNum + resNum - 10, midLinSym) << lineEnd << endl;
  ...
  report->file << lineBeg << " "
               << "band"
               << " " << delimiter2 << " "
               << "RMSE" << createStr(maxDigits[0] - 4, " ") << " "
               << delimiter1 << " "
               << "CC" << createStr(maxDigits[1] - 2, " ") << " " << delimiter1
               << " "
               << "ERGAS" << createStr(maxDigits[2] - 5, " ") << " "
               << delimiter1 << " "
               << "UIQI" << createStr(maxDigits[3] - 4, " ") << " " << lineEnd
               << " \n";

  report->file << lineBeg << createStr(firstColLen + 2, midLinSym) << delimiter2
               << createStr(fileAdj + resNumFile - 10, midLinSym) << lineEnd
               << "\n";
  ...
  for (int j = 0; j < glPrms->NChY; j++) {
    ...
    cout
        << lineBeg << createStr(firstColLen - bandStr.length(), " ")
        << j + 1 + dSetting->chBundleFirst << "  " << delimiter2 << " "
        << assessmStrSep[j].RMSE_sep_str
        << createStr(maxDigits[0] - assessmStrSep[j].RMSE_sep_str.length(), " ")
        << " " << delimiter1 << " " << assessmStrSep[j].CC_sep_str
        << createStr(maxDigits[1] - assessmStrSep[j].CC_sep_str.length(), " ")
        << " " << delimiter1 << " " << assessmStrSep[j].ERGAS_sep_str
        << createStr(maxDigits[2] - assessmStrSep[j].ERGAS_sep_str.length(),
                     " ")
        << " " << delimiter1 << " " << assessmStrSep[j].UIQI_sep_str
        << createStr(maxDigits[3] - assessmStrSep[j].UIQI_sep_str.length(), " ")
        << " " << delimiter1 << " " << assessmStrSep[j].DD_sep_str
        << createStr(maxDigits[4] - assessmStrSep[j].DD_sep_str.length(), " ")
        << " " << delimiter1 << " " << createStr(maxDigits[5], " ") << " "
        << delimiter1 << " " << createStr(maxDigits[6], " ") << " "
        << delimiter1 << " " << assessmStrSep[j].AG_orig_sep_str
        << createStr(maxDigits[7] - assessmStrSep[j].AG_orig_sep_str.length(),
                     " ")
        << " " << delimiter1 << " " << assessmStrSep[j].AG_rec_sep_str
        << createStr(maxDigits[8] - assessmStrSep[j].AG_rec_sep_str.length(),
                     " ")
        << " " << delimiter1 << " " << assessmStrSep[j].PSNR_sep_str
        << createStr(maxDigits[9] - assessmStrSep[j].PSNR_sep_str.length(), " ")
        << " " << lineEnd << endl;

    report->file
        << outLinSym << createStr(firstColLen - bandStr.length(), " ")
        << j + 1 + dSetting->chBundleFirst << "  " << delimiter2 << " "
        << assessmStrSep[j].RMSE_sep_str
        << createStr(maxDigits[0] - assessmStrSep[j].RMSE_sep_str.length(), " ")
        << " " << delimiter1 << " " << assessmStrSep[j].CC_sep_str
  ...
  cout << lineBeg << " "
       << "mean"
       << " " << delimiter2 << " " << assessmStr->RMSE_str
       << createStr(maxDigits[0] - assessmStr->RMSE_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->CC_str
       << createStr(maxDigits[1] - assessmStr->CC_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->ERGAS_str
       << createStr(maxDigits[2] - assessmStr->ERGAS_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->UIQI_str
       << createStr(maxDigits[3] - assessmStr->UIQI_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->DD_str
       << createStr(maxDigits[4] - assessmStr->DD_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->SAM_str
       << createStr(maxDigits[5] - assessmStr->SAM_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->DLambda_str
       << createStr(maxDigits[6] - assessmStr->DLambda_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->AG_orig_str
       << createStr(maxDigits[7] - assessmStr->AG_orig_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->AG_rec_str
       << createStr(maxDigits[8] - assessmStr->AG_rec_str.length(), " ") << " "
       << delimiter1 << " " << assessmStr->PSNR_str
       << createStr(maxDigits[9] - assessmStr->PSNR_str.length(), " ") << " "
       << lineEnd << endl;
  ...
  fileAdj = 0;
  fileAdj = maxDigits[4] + maxDigits[5] + maxDigits[6] + maxDigits[7] +
            maxDigits[8] + maxDigits[9];
  ...
  report->file << lineBeg << " "
               ...
               << "SAM" << createStr(maxDigits[5] - 3, " ") << " " << delimiter1
               << " "
               << "D_lambda" << createStr(maxDigits[6] - 8, " ") << " "
               << delimiter1 << " "
               << "AG_orig" << createStr(maxDigits[7] - 7, " ") << " "
               << delimiter1 << " "
               << "AG_rec" << createStr(maxDigits[8] - 6, " ") << " "
               << delimiter1 << " "
               << "PSNR" << createStr(maxDigits[9] - 4, " ") << " " << lineEnd
               << "\n";

  report->file << lineBeg << createStr(firstColLen + 2, midLinSym) << delimiter2
               << createStr(fileAdj + resNumFile2 - 10, midLinSym) << lineEnd
               << "\n";
  ...
  for (int jj = 0; jj < glPrms->NChY; jj++) {
    ...
    report->file
        << lineBeg << createStr(firstColLen - bandStr.length(), " ")
        << jj + 1 + dSetting->chBundleFirst << "  " << delimiter2 << " "
        << assessmStrSep[jj].DD_sep_str
        << createStr(maxDigits[4] - assessmStrSep[jj].DD_sep_str.length(), " ")
        << " " << delimiter1 << " " << createStr(maxDigits[5], " ") << " "
        << delimiter1 << " " << createStr(maxDigits[6], " ") << " "
        << delimiter1 << " " << assessmStrSep[jj].AG_orig_sep_str
        << createStr(maxDigits[7] - assessmStrSep[jj].AG_orig_sep_str.length(),
                     " ")
        << " " << delimiter1 << " " << assessmStrSep[jj].AG_rec_sep_str
        << createStr(maxDigits[8] - assessmStrSep[jj].AG_rec_sep_str.length(),
                     " ")
        << " " << delimiter1 << " " << assessmStrSep[jj].PSNR_sep_str
        << createStr(maxDigits[9] - assessmStrSep[jj].PSNR_sep_str.length(),
                     " ")
        << " " << lineEnd << "\n";
  }
  report->file << lineBeg << createStr(firstColLen + 2, midLinSym) << delimiter2
...
int read_CSV(SpEOMatrixD *Mat, const char *fname_CSV, char delimiter,
  ...
  if (CSV_file.is_open()) {
    ...
    while (CSV_file.good()) {
      if (CSVcol != m_row.size()) {
        cerr << endl
             << "[" << my_rank << "] ERROR while reading CSV in row number "
    ...
    *Mat = SpEOMatrixD::Zero(CSVrow, CSVcol);
    ...
    for (unsigned int r = 0; r < CSVrow; r++) {
      ...
      for (unsigned int c = 0; c < CSVcol; c++) {
        if (!(istringstream(m_row[c]) >> (*Mat)(r, c))) (*Mat)(r, c) = -999;
      }
    }
    return 1;
  }
  ...
  if (my_rank == 0) {
    cout << "[" << my_rank << "] ERROR: The file '" << fname_CSV
         << "' does not exist in the file system!" << endl;
    cerr << "[" << my_rank << "] ERROR: The file '" << fname_CSV
         << "' does not exist in the file system!" << endl;
  }
  return -1;
}
...
int read_CSV(SpEOMatrixI *Mat, const char *fname_CSV, char delimiter,
  ...
  std::ifstream CSV_file(
      fname_CSV);  // std::ifstream CSV_file(fname_CSV.c_str());
  if (CSV_file.is_open()) {
    unsigned int CSVrow = 0;
    for (int i = 0; i < skipLns; i++) {
      CSV_file.ignore(500, '\n');
...
void write_Mat_to_CSV(SpEOMatrixF *Mat, const char *fname_CSV) {
  ...
  if (CSV_file.is_open()) {
    ...
    cout << endl
         << "WARNING: Matrix could not be written to .csv file!" << endl;
  }
  CSV_file.close();
}
...
void write_Mat_to_CSV(SpEOMatrixD *Mat, const char *fname_CSV) {
  ...
  if (CSV_file.is_open()) {
    ...
    cout << endl
         << "WARNING: CSV_file '" << fname_CSV << "' could not be written!"
         << endl;
  }
  CSV_file.close();
...
void read_SRF(SpEOGlobalParams *glPrms, SpEOMatrixD *Mat, string fname_CSV,
  ...
  if (stat_CSV_read == -1) {
    if (my_rank == 0) {
      cout << "[" << my_rank << "] ERROR: The SRF file '" << fname_CSV.c_str()
           << "' does not exist in the file system!" << endl;
    }
    MPI_Barrier(MPI_COMM_WORLD);
    exit(2);
  ...
  if ((*Mat).minCoeff() < 0.0) {
    if (my_rank == 0) {
      cerr << endl
           << "ERROR: SRF is not allowed to contain negative values!" << endl
           << endl;
    }
    MPI_Barrier(MPI_COMM_WORLD);
    exit(2);
    ...
    if (Mat->rows() == glPrms->NChX_orig && Mat->cols() == glPrms->NChY_orig) {
      ...
      if (my_rank == 0) {
        cerr << endl
             << "ERROR: The SRF matrix stored in the file " << endl
             << endl
             << "   " << fname_CSV << endl
             << endl
             << "has the wrong dimensions! It must have as many rows as number "
                "of MS channels and as many columns as number of HS channels!"
             << endl
             << endl;
      }
      MPI_Barrier(MPI_COMM_WORLD);
  ...
  if (my_rank == 0) {
    cout << "Check sum-to-one constraint. I.e. check if the SRFs are "
            "individually normalized..."
         << endl;
    cout << "The sum of the SRF entries in channel iChX is differs from 1.0 "
            "(ideal case) as follows (should be equal to 0.0)!"
         << endl;
  }
  for (int iChX = 0; iChX < Mat->rows(); iChX++) {
    ...
    if (my_rank == 0) {
      cout << "iChX=" << iChX << "  ->  sum(SRF(" << iChX
           << "))-1 = " << sum - 1 << endl;
    }
  }
  // normalize each SRF (i.e. each row) so that the entries in each row sum up
  // to one
  if (normalize_SRF) {
    if (my_rank == 0) {
      cout << "normalize SRF, so that the sum in every row (i.e. for every "
              "channel of the image X) is equal to one..."
           << endl;
    }
    for (int iChX = 0; iChX < Mat->rows(); iChX++) {
      double sum = 0;
      ...
      if (sum < 1e-6) {
        if (my_rank == 0) {
          cout << "ERROR: the sum of the SRF entries in channel " << iChX
               << " is close to or equal to zero. Row-wise normalization would "
                  "create inf values in the SRF!"
               << endl;
        }
        MPI_Barrier(MPI_COMM_WORLD);
        exit(2);
...
void save_dSetting(SpEOPaths *paths, SpEODataIOSetting *dSetting) {
  ...
  cout << "write data I/O settings to files in directory: " << endl
       << "     " << dir_dSetting << " .. ";
...
void save_fSetting(SpEOPaths *paths, SpEOFusionSetting *fSetting) {
  ...
  cout << "write fusion settings to files in directory: " << endl
       << "     " << dir_fSetting << " .. ";
  ...
  if (fSetting_file_matlab.is_open()) {
    ...
    fSetting_file_matlab << "  fSet.nrmlDicts=" << fSetting->nrmlDicts << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.subtrMean=" << fSetting->substrMean << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.ImZ_ref_avlbl=" << fSetting->ImZ_ref_avlbl
                         << ";" << endl;
    fSetting_file_matlab << "  fSet.Nc=" << fSetting->Nc << ";" << endl;
    fSetting_file_matlab << "  fSet.No=" << fSetting->No << ";" << endl;
    fSetting_file_matlab << "  fSet.tol_SRF=" << fSetting->tol_SRF << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.Nc_max=" << fSetting->Nc_max << ";" << endl;
    fSetting_file_matlab << "  fSet.theta=" << fSetting->theta << ";" << endl;
    fSetting_file_matlab << "  fSet.patchsize=" << fSetting->patchsize << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.overlap=" << fSetting->overlap << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.lambda=" << fSetting->lambda << ";" << endl;
    fSetting_file_matlab << "  fSet.NDP=" << fSetting->NDP << ";" << endl;
    fSetting_file_matlab << "  fSet.two_step_estimation="
                         << fSetting->two_step_estimation << ";" << endl;
    fSetting_file_matlab << "  fSet.dictselect=" << fSetting->dictselect << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.matrixNorm=" << fSetting->matrixNorm << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.addMeanPixelwise="
                         << fSetting->addMeanPixelwise << ";" << endl;
    fSetting_file_matlab << "  fSet.lambdaX=" << fSetting->lambdaX << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.lambdaY=" << fSetting->lambdaY << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.LQ_post_opt_im=" << fSetting->LQ_post_opt_im
                         << ";" << endl;
    fSetting_file_matlab << "  fSet.lambdaX_im=" << fSetting->lambdaX_im << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.lambdaY_im=" << fSetting->lambdaY_im << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.iterMain=" << fSetting->iterMain << ";"
                         << endl;
    fSetting_file_matlab << "  fSet.doFullImOptWithoutPatRec="
                         << fSetting->doFullImOptWithoutPatRec << ";" << endl;
    fSetting_file_matlab << "  fSet.set_neg_to_0=" << fSetting->set_neg_to_0
                         << ";" << endl;
    fSetting_file_matlab << "  fSet.use_estimated_SRFs="
                         << fSetting->use_estimated_SRFs << ";" << endl;
    fSetting_file_matlab << "  fSet.ImX_sim_mode=" << fSetting->ImX_sim_mode
                         << ";" << endl;
    fSetting_file_matlab << "  fSet.use_LRnorm_for_dic_normalization="
                         << fSetting->use_LRnorm_for_dic_normalization << ";"
                         << endl;
    cout << endl << "fusionSettings.m written." << endl;
  } else {
    cout << endl
         << "WARNING: statistics_matlab.m could not be written!" << endl;
  }
...
void save_oSetting(SpEOPaths *paths, SpEOOutputSetting *oSetting) {
  ...
  cout << "write output settings to files in directory: " << endl
       << "     " << dir_oSetting << " .. ";
...
void save_sSetting(SpEOPaths *paths, SpEOSolverSetting *sSetting) {
  ...
  cout << "write solver settings to files in directory: " << endl
       << "     " << dir_sSetting << " .. ";
...
void save_pSetting(SpEOPaths *paths, SpEOParallelSetting *pSetting) {
  ...
  cout << "write parallelization settings to files in directory: " << endl
       << "     " << dir_pSetting << " .. ";
...
void save_glPrms(SpEOPaths *paths, SpEOGlobalParams *glPrms) {
  ...
  cout << "write global parameters to files in directory: " << endl
       << "     " << dir_glPrms << " .. ";
...
void save_fusion_setup(SpEOPaths *paths, SpEODataIOSetting *dSetting,
  ...
  cout << "write fusion setup to file: " << endl
       << "     " << fname_fusionSetup << " .. ";
  ...
  if (fusionSetup_file_matlab.is_open()) {
    ...
    fusionSetup_file_matlab
        << "function [fSet,dSet,sSet,oSet,glPrms,paths]=fusionSetup(~)" << endl;
    fusionSetup_file_matlab
        << "%% Fusion Settings" << endl
        << "fSet.fMethod=" << fSetting->fMethod << ";" << endl
        << "fSet.two_step_estimation=" << fSetting->two_step_estimation << ";"
        << endl
        ...
        << "fSet.evaluate_ImZ_init=" << fSetting->evaluate_ImZ_init << ";"
        << endl
        ...
        << "fSet.lambdaZ_ABC_in_1st_iter=" << fSetting->lambdaZ_ABC_in_1st_iter
        << ";" << endl;
    fusionSetup_file_matlab
        << "fSet.iterMain=" << fSetting->iterMain << ";" << endl
        << "fSet.doFullImOptWithoutPatRec="
        << fSetting->doFullImOptWithoutPatRec << ";" << endl
        << "fSet.Nc_max=" << fSetting->Nc_max << ";" << endl
        << "fSet.theta=" << fSetting->theta << ";" << endl
        << "fSet.set_neg_to_0=" << fSetting->set_neg_to_0 << ";" << endl
        << "fSet.use_estimated_SRFs=" << fSetting->use_estimated_SRFs << ";"
        << endl
        << "fSet.fullImOptOnSubspace=" << fSetting->fullImOptOnSubspace << ";"
        << endl
        << "fSet.use_init_value_Eq1Unmixing="
        << fSetting->use_init_value_Eq1Unmixing << ";" << endl
        << "fSet.subspace_transform_type=\'"
        << fSetting->subspace_transform_type << "\';" << endl
        << "fSet.subspace_dim=" << fSetting->subspace_dim << ";" << endl
        << "fSet.ImX_sim_mode=" << fSetting->ImX_sim_mode << ";" << endl
        << "fSet.SNR_normalization=" << fSetting->SNR_normalization << ";"
        << endl
        << "fSet.balance_ImX_term_coef=" << fSetting->balance_ImX_term_coef
        << ";" << endl
        << endl;

    fusionSetup_file_matlab
        << "%% dataIO Settings" << endl
        ...
        << endl;
    fusionSetup_file_matlab
        << "%% Parrallilization Settings" << endl
        ...
        << "pSet.workStealingTurns=" << pSetting->workStealingTurns << ";"
        << endl
        << "pSet.store_patches_tmp_on_drive="
        << pSetting->store_patches_tmp_on_drive << ";" << endl
        << "pSet.parWrNumProc=" << pSetting->parWrNumProc << ";" << endl
        << endl;
    fusionSetup_file_matlab
        << "%% Solver Settings" << endl
    ...
    fusionSetup_file_matlab
        ...
        << "oSet.writeImageFileAfterEveryIter="
        << oSetting->writeImageFileAfterEveryIter << ";" << endl
        << endl;
    fusionSetup_file_matlab
        ...
        << endl;
    fusionSetup_file_matlab << "%% paths" << endl << endl;
...
void save_evalResults(SpEOPaths *paths, SpEOGlobalParams *glPrms,
  ...
  cout << "write assessment results to files in directory: " << endl
       << "     " << dir_eval << " .. ";
  ...
  if (eval_file_matlab.is_open()) {
    ...
    eval_file_matlab
        << "  e.RMSE_mean=" << assMetrics_HR->RMSE_mean << ";" << endl
...
// ====================================
/* Select dictionary atoms on the basis of Euclidean Norm comparison for ImX_LR,
 * ImX, and ImY patches
 */
...
void dictSelectFunc(SpEOMatrixF *patchCompSubsetfloat, SpEODataset *ImX_LR,
                    ...
                    SpEOMatrixF *patchComp, int my_rank, int ipp,
                    int &NDP) {  // Current Patch and Comparison Patch
  ...
  switch (fSet->dictselect) {
    case 0: {  // Case (0): dictionary contains only current patch under
               // reconstruction
      sortON = false;
    ...
    case 1: {  // Case (1): Nearest Neighbor patches according to the maximum
               // norm of the patch coordinates
      sortON = false;
      (*patchCompSubsetfloat)(0, 0) = uP;
      (*patchCompSubsetfloat)(0, 1) = vP;
      int pntCnt = 0;
      ...
      while (k < NDP && tot_patch_cnt < glPrms->NP_sub) {
        // make a step, add 'direction' vector (du, dv) to current position (u,
        // v)
        u += du;
        v += dv;
        ++segment_passed;
        ...
        if (u >= 0 && u <= glPrms->NPU - 1 && v >= 0 && v <= glPrms->NPV - 1) {
          ...
          for (int ig = 0; ig < ImX->get_NCh(); ig++) {
            SpEOMatrixD patchHR_tmp =
                (ImX->get_rasterBands()[ig]->bandDataMatD.block(
                    idxPUH->coeff(u), idxPVH->coeff(v), pszH, pszH));
            SpEOMatrixD patchLR_tmp =
                (ImX_LR->get_rasterBands()[ig]->bandDataMatD.block(
                    idxPUL->coeff(u), idxPVL->coeff(v), pszL, pszL));
            if (patchHR_tmp.norm() < 1e-8 || patchLR_tmp.norm() < 1e-8) {
              patch_of_zeros_found = true;
              cout << "In either ImX_sim or in ImX_sim_LR: patch (uP,vP)=("
                   << uP << "," << vP << ") inf band/group ig=" << ig
                   << " contains only zeros and was therefore excluded from "
                      "the dictionary!"
                   << endl;
            }
          }
    ...
    case 2: {  // Case (2): Compute norms based on |ImX_LRcurrent-ImX_LR|
      // Set the currP as the current patch in the ImX_LR image, for norm
      // comparison.
      sortON = true;
      currP = ImX_LR->get_rasterBands()[iG]->get_bandDataMat()->block(
          (*idxPUL)(uP), (*idxPVL)(vP), pszL, pszL);
      ...
      for (int iN = 0; iN < NP; iN++) {
        ...
        int uN = iN / NPV;
        int vN = iN % NPV;
        compP = ImX_LR->get_rasterBands()[iG]->get_bandDataMat()->block(
            (*idxPUL)(uN), (*idxPVL)(vN), pszL, pszL);
    ...
    case 3: {  // Case (3): Compute norms based on |ImX_approx-ImX|, ImX_approx
               // = sum_Nc(ImY_channel*SRF_channel)
      // Use sum of ImY_channel*SRF_channel to form LR reconstruction as the
      // currP
      sortON = true;
      for (int channel = 0; channel < fSet->Nc; channel++) {
        currenttmp = (ImY->get_rasterBands()[channel]->get_bandDataMat()->block(
                         (*idxPUL)(uP), (*idxPVL)(vP), pszL, pszL)) *
                     (*SRF)(0, channel);
    ...
    case 4: {  // Case(4): HR norm comparison
      // Set the currP as the current patch in the ImX_HR image, for norm
      // comparison.
      sortON = true;
      currP.resize(pszH, pszH);
      compP.resize(pszH, pszH);
      currP = ImX->get_rasterBands()[iG]->get_bandDataMat()->block(
          (*idxPUH)(uP), (*idxPVH)(vP), pszH, pszH);
      // Arrange current patch representation, currP, into a vector.
      ...
      for (int iN = 0; iN < NP; iN++) {
        int uN = iN / NPV;
        int vN = iN % NPV;
        // Case of HR Correlation comparison (less efficient than HR norm
        // Comparison) Get Indices for all NP patches
        compP = ImX->get_rasterBands()[iG]->get_bandDataMat()->block(
            (*idxPUH)(uN), (*idxPVH)(vN), pszH, pszH);
    ...
    case 5: {  // Case(5): HR & LR Norm combined ranking !!! Inefficient !!!
      ...
      // Generate matrix of indices and norms, in ascending order according to
      // the Norm column
      LRCompOrdered = sort_orderLR.asPermutation() * LRComp;
      // In case the user wants to check the norms and ordering of patches are
      // appropriate print the lines below:
      ...
      for (int i = 0; i < NP; i++) {
        // Compute combined ranking = sum of the ordered HR and LR row positions
        // for each patch.
        for (int j = 0; j < NP; j++) {
          if (LRCompOrdered.row(i).col(0) == HRCompOrdered.row(j).col(0) &&
              LRCompOrdered.row(i).col(1) == HRCompOrdered.row(j).col(1)) {
    ...
    case 6: {  // Case (6): Compute metric based on ABSOLUTE correlation between
               // ImX_HRcurrent and ImX_HR
      // Set the currP as the current patch in the ImX_HR image, for norm
      // comparison.
      sortON = true;
      currP.resize(pszH, pszH);
    ...
    case 7: {  // Case (7): Compute metric based on **MINIMUM** ABSOLUTE
               // correlation between ImX_HRcurrent and ImX_HR
      // Set the currP as the current patch in the ImX_HR image, for norm
      // comparison.
      sortON = true;
      currP.resize(pszH, pszH);
    ...
    case 8: {  // Case (8): Compute metric based on random patch subset, though
               // retaining the current patch as first dictionary atom
      ...
      for (int iD = 1; iD < NDP; iD++) {
        // Compute pseudo hashing function for seed (this could be improved by
        // including processor rank).
        int time = MPI_Wtime();
        int seed =
            abs(time * 121 - my_rank * 12345 + iP * iD - uP + vP + iG * 79) %
            104729;
        // Seed random number
        srand(seed);
    ...
    case 9: {  // Case (9): Uncorrelated dictionary - i.e dictionary  attempts
               // not to self correlate, to approximate a basis
      // WARNING: Case 9 involves an extremely expensive combinatorial search -
      // NOT RECOMMENDED
      sortON = false;
      currP.resize(pszH, pszH);
      compP.resize(pszH, pszH);
      ...
      for (int iD = 1; iD < NDP; iD++) {
        // sum: net summation of correlation coefficients between patch iN and
        // current patches in the dictionary
        float sum = 0;
        ...
        // Loop Through all patches - find the net correlation of each patch to
        // the current dictionary
        for (int iN = 0; iN < NP; iN++) {
          int uN = iN / NPV;
          int vN = iN % NPV;
          sum = 0;
          // anyof: switch deciding whether the patch iN is any of the patches
          // in the dictionary
          int anyof = 0;
          ...
          if (anyof == 0) {
            ...
            compV /= norm_tmp;
            for (int sP = 0; sP < iD; sP++) {
              // Find correlation between compare patch and patches that are
              // ALREADY SELECTED IN THE DICTIONARY Sum these correlations iiN:
              // index of patch inside the vector selectedPatches
              int iiN = selectedPatches(sP);
              int uiN = iiN / NPV;
              int viN = iiN % NPV;
              SpEOMatrixF alreadySelectedP =
                  ImX->get_rasterBands()[iG]->get_bandDataMat()->block(
                      (*idxPUH)(uiN), (*idxPVH)(viN), pszH, pszH);
              SpEOVectorF alreadySelectedV =
                  SpEOVectorF::Map(alreadySelectedP.data(), pszH2);
              alreadySelectedV.array() -= compV.mean();
              norm_tmp = alreadySelectedV.norm();
              alreadySelectedV /= norm_tmp;

              // find the absolute correlation between the patch in question and
              // the current patch ALREADY SELECTED IN THE DICTIONARY
    ...
    default: {
      sortON = false;
      if (my_rank == 0 && iP == 0) {
        cout << "##### WARNING: Unknown dictionary selection method: "
             << fSet->dictselect
             << "! Choose a method between 0 and 8! The nearest neighbour "
                "dictionary selection method is used as the default!"
             << endl;
      }
  ...
  // ##### Sorting #####
  // Order the patchComp matrix of the form
  // |NPIndices.coeff(0)|NPIndices.coeff(1)|Norm| such that the metric column is
  ...
  if (sortON == true) {
    ...
    if (fSet->dictselect == 4) {
      ...
      // Take first NDP rows of Nonlocal patchCompOrdered table to form
      // dictionary of size NDP
      *patchCompSubsetfloat = patchCompOrdered.block(0, 0, NDP, 3);
    } else {
      VectorXi sort_order(NP);
      std::vector<argsort_pair> data(NP);
...
void dictSelectFunc(SpEOMatrixD *patchCompSubsetDouble, SpEODataset *ImX_LR,
                    SpEODataset *ImX, SpEODataset *ImY, SpEOFusionSetting *fSet,
                    SpEOGlobalParams *glPrms, int NP, int iP, int uP, int vP,
                    SpEOVectorI *idxPUH, SpEOVectorI *idxPVH,
                    SpEOVectorI *idxPUL, SpEOVectorI *idxPVL, SpEOMatrixD *SRF,
  ...
  int iG = ipp;
  switch (fSet->dictselect) {
    case 0: {  // Case (1): Nearest Neighbor patches according to the maximum
               // norm of the patch coordinates
      sortON = false;
    ...
    case 2: {  // Case (2): Compute norms based on |ImX_LRcurrent-ImX_LR|
      ...
      // comparison.
      sortON = true;
      currP = ImX_LR->get_rasterBands()[iG]->bandDataMatD.block(
      ...
      for (int iN = 0; iN < NP; iN++) {
        ...
        int uN = iN / NPV;
        int vN = iN % NPV;
        compP = ImX_LR->get_rasterBands()[iG]->bandDataMatD.block(
            idxPUL->coeff(uN), idxPVL->coeff(vN), pszL, pszL);
    ...
    case 3: {  // Case (3): Compute norms based on |ImX_approx-ImX|, ImX_approx
      ...
      for (int channel = 0; channel < fSet->Nc; channel++) {
        SpEOMatrixF tttmp =
            (ImY->get_rasterBands()[channel]->get_bandDataMat()->block(
                (*idxPUL)(uP), (*idxPVL)(vP), pszL, pszL));
    ...
    case 4: {  // Case(4): HR norm comparison
      ...
      currP.resize(pszH, pszH);
      compP.resize(pszH, pszH);
      currP = ImX->get_rasterBands()[iG]->bandDataMatD.block(
          (*idxPUH)(uP), (*idxPVH)(vP), pszH, pszH);
      // Arrange current patch representation, currP, into a vector.
      ...
      for (int iN = 0; iN < NP; iN++) {
        ...
        // Case of HR Correlation comparison (less efficient than HR norm
        // Comparison) Get Indices for all NP patches
        compP = ImX->get_rasterBands()[iG]->bandDataMatD.block(
            (*idxPUH)(uN), (*idxPVH)(vN), pszH, pszH);
    ...
    case 8: {  // Case (8): Compute metric based on random patch subset, though
      ...
      for (int iD = 1; iD < NDP; iD++) {
        ...
        // including processor rank).
        int time = MPI_Wtime();
        int seed = abs(time * 121 - (my_rank + 1) * 12345 + (iP + 1) * iD - uP +
                       vP + (iG + 1) * 79) %
                   104729;
        // Seed random number
        srand(seed);
        ...
        for (int iN = 0; iN < NP; iN++) {
          ...
          if (anyof == 0) {
            ...
            for (int sP = 0; sP < iD; sP++) {
              ...
              int uiN = iiN / NPV;
              int viN = iiN % NPV;
              SpEOMatrixD alreadySelectedP =
                  ImX->get_rasterBands()[iG]->bandDataMatD.block(
                      (*idxPUH)(uiN), (*idxPVH)(viN), pszH, pszH);
              SpEOVectorD alreadySelectedV =
                  SpEOVectorD::Map(alreadySelectedP.data(), pszH2);
              alreadySelectedV.array() -= compV.mean();
              norm_tmp = alreadySelectedV.norm();
              alreadySelectedV /= norm_tmp;
              // find the absolute correlation between the patch in question and
...
bool is_inf_or_nan(double x) { return !(x - x == x - x); }
bool is_inf_or_nan(float x) { return !(x - x == x - x); }
bool is_inf_or_nan(int x) { return !(x - x == x - x); }
...
bool contains_nan(SpEOMatrixD x) { return !((x.array() == x.array()).all()); }

bool contains_nan(SpEOVectorD x) { return !((x.array() == x.array()).all()); }
...
double calc_Corr_Coeff(SpEOMatrixD mat1, SpEOMatrixD mat2) {
  ...
  double CC = (mat1_zrMn.transpose() * mat2_zrMn)(0, 0) /
              (sqrt((mat1_zrMn.transpose() * mat1_zrMn)(0, 0)) *
               sqrt((mat2_zrMn.transpose() * mat2_zrMn)(0, 0)));
  return CC;
}
...
void lowPassFilter_and_downSample(SpEODataset *Im, SpEODataset *Im_LR,
  ...
  for (int iCh = 0; iCh < NCh; iCh++) {
    ...
    if (input_format == SpEODouble) {
      ...
      mat_HR_tmp =
          Im->get_rasterBands()[iCh]->get_bandDataMat()->cast<double>();
    }
    fast_filter(mat_LR_tmp, mat_HR_tmp, gauss_filter, filter_coeff,
                glPrms.fDS);  // fast method (intuitive)
...
void calc_ImX_sim(SpEODataset *ImX_sim, SpEODataset *ImX, SpEODataset *ImX_LR,
  ...
  MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
  int chBundleFirst_tmp = dSetting->chBundleFirst;
  dSetting->chBundleFirst = 0;
  int chBundleLast_tmp;
  chBundleLast_tmp = dSetting->chBundleLast;
  dSetting->chBundleLast = Ng - 1;
  int sizeUH_red_tmp = glPrms->sizeUH_red;
  glPrms->sizeUH_red = glPrms->sizeUH;
  int sizeVH_red_tmp = glPrms->sizeVH_red;
  glPrms->sizeVH_red = glPrms->sizeVH;
  ...
  int idxWUL =
      max(0, idxPUL->coeff(uP) -
                 (int)(0.5 * (double)(winSizeL - fSetting->patchsize)));
  int idxWVL =
      max(0, idxPVL->coeff(vP) -
                 (int)(0.5 * (double)(winSizeL - fSetting->patchsize)));
  int winSizeUL =
      min((int)ImY->get_sizeU() - 1,
          idxPUL->coeff(uP) -
              (int)(0.5 * (double)(winSizeL - fSetting->patchsize)) + winSizeL -
              1) -
      idxWUL + 1;
  int winSizeVL =
  ...
  for (int iG = 0; iG < Ng; iG++) {
    ...
    double c_hat[glPrms->NChX];  // this is the vector of weights for different
                                 // ImX bands estimated via one of the 3
                                 // following modes
    ...
    switch (sim_mode) {
      case 0: {
        if (localCalculation) {
          ...
          // if all CCs are close to zero, then the corresponding band in
          // ImX_sim will contain only entries that are close to zero which in
          // turn results in dictionary atoms which have norm close to zero.
          // This would cause NaN numbers as the dictionary atoms will be (and
          // have to be) normalized. Hence, we enlarge the window size for this
          // group until this problem is resolved
          if (CCs.maxCoeff() < (1e-8) && CCs.minCoeff() > -(1e-8)) {
            ...
            cout << "         [" << my_rank << "] WWWW Warning:"
                 << " iG=" << iG << " [ImY bands " << idxChY[iG] << " to "
                 << idxChY[iG] + Nc_vec[iG] - 1
                 << "] -> all CCs (and probably the entire ImY patch in this "
            ...
            do {
              ...
              idxWVL = max(
                  0, idxPVL->coeff(vP) -
                         (int)(0.5 * (double)(winSizeL - fSetting->patchsize)));
              winSizeUL = min((int)ImY->get_sizeU() - 1,
                              idxPUL->coeff(uP) -
                                  (int)(0.5 * (double)(winSizeL -
                                                       fSetting->patchsize)) +
                                  winSizeL - 1) -
                          idxWUL + 1;
              winSizeVL = min((int)ImY->get_sizeV() - 1,
                              idxPVL->coeff(vP) -
                                  (int)(0.5 * (double)(winSizeL -
                                                       fSetting->patchsize)) +
                                  winSizeL - 1) -
              ...
              check_for_inf_or_nan(my_rank, patX_LR_mod, " ", -123,
                                   "patX_LR_mod");
              ...
              if (winSizeUL > ImY->get_sizeU() &&
                  winSizeVL > ImY->get_sizeV()) {
                cerr << "         EEEE Error: Error occurred while enlarging "
                        "the window for generating ImX_sim.."
                     << endl
                     << "                     window exceeds the size of the "
                        "image, which can only mean that "
                     << endl
                     << "                     ImY contains only zeros in the "
                        "current group (iG="
                     << iG << ") of bands.." << endl
                     << "                     In fact, ImY_g_avg_vec.norm() = "
                     << ImY_g_avg_vec.norm() << endl
                     << endl;
                break;
              }
            } while ((CCs.maxCoeff() < (1e-8) && CCs.minCoeff() > -(1e-8)) ||
                     !extraRoundCompleted);
            cout << endl;
          }
        } else {
          SpEOMatrixD ImY_g_avg =
              SpEOMatrixD::Zero(glPrms->sizeUL, glPrms->sizeVL);
          int iC, iChY;
          for (iC = 0; iC < Nc_vec[iG]; iC++) {
            iChY = idxChY[iG] + iC;
            SpEOMatrixD ttmp =
          ...
          for (iChX = 0; iChX < glPrms->NChX; iChX++) {
            SpEOMatrixD ttmp = ImX_LR->get_rasterBands()[iChX]
                                   ->get_bandDataMat()
                                   ->cast<double>();
            ttmp.array() -= ttmp.mean();
            ttmp.array() /= ttmp.norm();
          ...
          ImY_g_avg_vec = SpEOVectorD::Map(ImY_g_avg.data(),
                                           ImY_g_avg.rows() * ImY_g_avg.cols());
        }
        // sort the CCs in a decreasing manner to find
        // b_tilde[0],...,b_tilde[NChX-1]
        ...
        if (localCalculation) {
          ...
          SpEOMatrixD mat2 = ImX_LR->get_rasterBands()[b_tilde[0]]
                                 ->get_bandDataMat()
                                 ->cast<double>();
        ...
        c_hat[0] = 1 / (vec2.dot(vec2)) *
                   vec2.dot(ImY_g_avg_vec);  // least squares solution
        ...
        for (iChX = 1; iChX < glPrms->NChX; iChX++) {
          ...
          if (localCalculation) {
            ...
            SpEOMatrixD mat3 = ImX_LR->get_rasterBands()[b_tilde[iChX]]
                                   ->get_bandDataMat()
                                   ->cast<double>();
          ...
          c_hat[iChX] = 1 / (vec3.dot(vec3)) *
                        vec3.dot(ImY_g_avg_vec -
                                 sum_ci_ImXLR_vec);  // least squares solution
      ...
      case 1: {  // unconstrained least squares
        ...
        SpEOMatrixD winY_g =
            SpEOMatrixD::Zero(Nc_vec[iG], winSizeUL * winSizeVL);
        int iC;
        for (iC = 0; iC < Nc_vec[iG]; iC++) {
          iChY = idxChY[iG] + iC;
          SpEOMatrixD windowBand =
        ...
        SpEOMatrixD winX_LR =
            SpEOMatrixD::Zero(glPrms->NChX, winSizeUL * winSizeVL);
        for (iChX = 0; iChX < glPrms->NChX; iChX++) {
          SpEOMatrixD windowBand =
              (ImX_LR->get_rasterBands()[iChX]->get_bandDataMat()->block(
                   idxWUL, idxWVL, winSizeUL, winSizeVL))
      ...
      case 2: {  // nonnegative least squares
        ...
        if (!all_good) {
          cout << "[" << my_rank << "] WARNING: NNLS failed at uP=" << uP
               << " vP=" << vP << " iG=" << iG << endl
               << "The problem failed to solve was Ax=b where x>=0 and "
               << "A = " << A << endl
               << endl
               << "b'=" << endl
               << b.transpose() << endl;
    ...
    // simulate HR image band ImX_sim so that ImX_sim*B*S is (locally at current
    // patch) highly correlated with ImY_g_avg.
    SpEOMatrixD ImX_sim_band =
        SpEOMatrixD::Zero(ImX->get_rasterBands()[0]->get_bandDataMat()->rows(),
                          ImX->get_rasterBands()[0]->get_bandDataMat()->cols());
    for (int iChX = 0; iChX < glPrms->NChX; iChX++) {
      ImX_sim_band += c_hat[iChX] * ImX->get_rasterBands()[b_tilde[iChX]]
                                        ->get_bandDataMat()
                                        ->cast<double>();
    }
...
void calc_P_matrices(SpEOVectorD *P_lmd_vecs_loc, int **P_lmd_idx_bl_loc,
  ...
  // for every row (each corresponding to one HS channel iChY) calculate the
  // relevant corresponding block indexes (first row) and the corresponding
  // entry indexes relative to the corresponding block's origin each starting at
  // 0. first the number of non-trivial entries in each row (iChZ=0,...,NChZ-1)
  // needs to be counted
...
void CSG_corr_based_spectral_grouping(int &Ng_final, int *idxChY_final,
                                      int *Nc_vec_final, SpEOMatrixD &winY,
                                      double CCmin, int Nc_max, int my_rank) {
  ...
  if (my_rank == 0 && printOutAll) {
    cout << "CCmat1 = " << endl << CCmat << endl << endl;
  }
  //##################################################################################
  ...
  if (my_rank == 0 && printOutAll) {
    cout << "CCmat2 = " << endl << CCmat << endl << endl;
  }
  //##################################################################################
  //#     3) “Clean up” CC_mat matrix by setting all entries (i,j) to -2 for
  //which   # #        there is an entry CC_mat(k,l) = -2  s.t. # # EITHER l=j
  //&& j<k<i                                                # #        OR k=i &&
  //i<l<j                                                # #        This step
  //limits the relevant entries in the matrix to possibly         # #
  //overlapping blocks along the diagonal.                                  #
  //##################################################################################
  cleanUp_CCmat(CCmat, q);
  if (my_rank == 0 && printOutAll) {
    cout << "CCmat3 = " << endl << CCmat << endl << endl;
  }
  //##################################################################################
  //#     4) Identify all blocks along the diagonal and associate every block
  //with a # #        group of adjacent bands. Moreover, distinguish two types
  //of groups:     # #        - Groups that are composed of fewer than or equal
  //to NC,max bands       # #          (colored in green) and # #        -
  ...
  //##################################################################################
  //#     5) Split each large group into possibly overlapping smaller sub-groups
  //as  # #        follows: # #        a) Refine the pre-defined threshold CCmin
  //(for each group individually) # #           by calculating the following
  //three potential thresholds of which the # #           smallest one will be
  //selected (as a minimum requirement so to say):  # #           Let A be the
  //number of all entries in the upper half (incl. the      # # diagonal) of the
  //symmetric current large block in CC_mat.            # #           i)
  //Calculating CCmin,1: Let B<A be the number of those entries      # # (i,j)
  //in the upper half (incl. the diagonal) of the symmetric    # # current large
  //block in CC_mat that are less than or equal to     # #               NC,max
  //entries away from the diagonal, i.e. i <= j < i+NC,max or # # j <= i <
  //j+NC,max. Next, put this upper part of the current      # # block in a
  //vector VEC (of length A) and sort it according to its # # entries
  //(correlation values) in a descending manner.             # # Set CCmin,1 =
  //VEC(B). In other words, CCmin,1 is the Bth highest # # correlation value in
  //the upper half of the current correlation   # #               sub-matrix. #
  //#           ii) Calculating CCmin,2: CCmin,2 = VEC(k*), where k* is
  //calculated   # #               via # #               sum_k=1^(k*) VEC(k)
  //approx.= (1/10)*sum_k=1^A VEC(k)             # #           iii) Calculating
  //CCmin,3: CCmin,3 = CCmin + (2/3)*(1-CCmin)          # #        b) - Select
  //the smallest of the three potential thresholds. This way all# # of the
  //conditions 5)a)i)-iii) are sufficient, but only the weakest # # one is
  //necessary (which one can vary between datasets, patches and # # even
  //blocks).                                                      # # -
  ...
  //occurring single band 1<=i<=NChY with the neighboring band j=i-1   # # or
  //j=i+1 that has the higher correlation with channel i.           # # #
  //##################################################################################
  int iG;
  ...
  for (iG = 0; iG < Ng; iG++) {
    if (Nc_vec[iG] > Nc_max) {
      ...
      SpEOMatrixD CCmat_grp =
          CCmat.block(idxChY[iG], idxChY[iG], Nc_vec[iG], Nc_vec[iG]);
      if (my_rank == 0 && printOutAll) {
        cout << "CCmat_grp = " << endl << CCmat_grp << endl << endl;
      }
      ...
      if (my_rank == 0 && printOutAll) {
        cout << "CCmin_grp = " << CCmin_grp << endl << endl;
      }
      threshold_CCmat(CCmat_grp, CCmin_grp, q);
      if (my_rank == 0 && printOutAll) {
        cout << "thresholded CCmat_grp = " << endl << CCmat_grp << endl << endl;
      }
      cleanUp_CCmat(CCmat_grp, q);
      if (my_rank == 0 && printOutAll) {
        cout << "thresholded cleaned CCmat_grp = " << endl
             << CCmat_grp << endl
             << endl;
      }
      ...
      calc_idxChY_and_Nc_vec(idxChY_grp[iG_lrg], Nc_vec_grp[iG_lrg],
                             Ng_grp[iG_lrg], CCmat_grp, q, idxChY[iG]);

      if (my_rank == 0 && printOutAll) {
        cout << "Ng_grp[iG_lrg=" << iG_lrg << "] = " << Ng_grp[iG_lrg] << endl;
      }
      ...
      grp_sngl_bnd_grps_with_high_corr_nbrs(Nc_vec_grp, idxChY_grp, Ng_grp,
                                            iG_lrg, CCmat);

      if (my_rank == 0 && printOutAll) {
        cout << endl
             << "After single band groups were groupt with high correlated "
                "neighbors:"
             << endl;
      }
      for (int iCh = 0; iCh < Ng_grp[iG_lrg]; iCh++) {
  ...
  for (iG_lrg = 0; iG_lrg < num_large_grps; iG_lrg++) {
    if (my_rank == 0 && printOutAll) {
      cout << "iG_lrg=" << iG_lrg
           << ", iG_lrg_all[iG_lrg]=" << iG_lrg_all[iG_lrg] << endl;
    }
  ...
  for (iG_sml = 0; iG_sml < num_sml_grps; iG_sml++) {
    if (my_rank == 0 && printOutAll) {
      cout << "iG_sml=" << iG_sml
           << ", iG_sml_all[iG_sml]=" << iG_sml_all[iG_sml] << endl;
    }
  }
  ...
  //##################################################################################
  //#     6) Remove redundant groups that might have been produced by
  //individually   # #        splitting possibly overlapping large groups in
  //step. More precisely,    # #        remove every group that is entirely
  ...
  for (iG_sml = 0; iG_sml < num_sml_grps; iG_sml++) {
    ...
    if (my_rank == 0 && printOutAll) {
      cout << "&  iG_all_grps=" << iG_all_grps
           << ": idxChY[iG_sml_all[iG_sml=" << iG_sml
           << "]=" << iG_sml_all[iG_sml] << "] = " << idxChY[iG_sml_all[iG_sml]]
  ...
  for (iG_lrg = 0; iG_lrg < num_large_grps; iG_lrg++) {
    for (int iG_sub = 0; iG_sub < Ng_grp[iG_lrg]; iG_sub++) {
      ...
      if (my_rank == 0 && printOutAll) {
        cout << "&  iG_all_grps=" << iG_all_grps
             << ": idxChY_grp[iG_lrg=" << iG_lrg << "][iG_sub=" << iG_sub
             << "] = " << idxChY_grp[iG_lrg][iG_sub] << "  #  "
  ...
  if (Ng_all_grps != iG_all_grps + 1) {
    cerr << "Ng_all_grps==" << Ng_all_grps
         << " != iG_all_grps+1==" << iG_all_grps + 1 << " !!! -> Fatal Error!"
         << endl
         << endl
         << endl;
    exit(2);
  ...
  for (iG_all_grps = 0; iG_all_grps < Ng_all_grps; iG_all_grps++) {
    if (my_rank == 0 && printOutAll) {
      cout << "&  all_grps_idxChY[iG_all_grps==" << iG_all_grps << "] = ("
           << all_grps_idxChY[iG_all_grps].first << " ,"
           << all_grps_idxChY[iG_all_grps].second << ")  #  "
           << all_grps_Nc_vec[iG_all_grps] << " = Nc_vec" << endl;
    }
  }
  ...
  while (iG_all_grps < Ng_all_grps) {
    // index if largest of all groups starting at
    // all_grps_idxChY[iG_all_grps].second
    ...
    while (iG_all_grps_run < Ng_all_grps &&
           (int)(all_grps_idxChY[iG_all_grps_run].second) == curr_frst_bnd) {
    ...
    for (int iG_all_grps_run_tmp = iG_all_grps;
         iG_all_grps_run_tmp < iG_all_grps_run; iG_all_grps_run_tmp++) {
  ...
  if (my_rank == 0 && printOutAll) {
    cout << endl
         << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" << endl
         << "&   after sorting and identifying redundant groups that START "
            "with the same band (keep only largest)"
         << endl
         << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" << endl;
  }
  for (iG_all_grps = 0; iG_all_grps < Ng_all_grps; iG_all_grps++) {
  ...
  for (iG_all_grps = 0; iG_all_grps < Ng_all_grps; iG_all_grps++) {
    if (grps_to_keep[iG_all_grps]) {
      // check if group contains other groups. If so, mark as "to be removed" /
      // "not to keep"
      iG_all_grps_run = iG_all_grps + 1;
      while (iG_all_grps_run < Ng_all_grps &&
             all_grps_idxChY[iG_all_grps_run].second <
                 all_grps_idxChY[iG_all_grps].second +
                     (float)(all_grps_Nc_vec[all_grps_idxChY[iG_all_grps]
                                                 .first])) {
        if (grps_to_keep[iG_all_grps_run] &&
            all_grps_idxChY[iG_all_grps_run].second +
                    (float)(all_grps_Nc_vec[all_grps_idxChY[iG_all_grps_run]
                                                .first]) <=
                all_grps_idxChY[iG_all_grps].second +
                    (float)(all_grps_Nc_vec[all_grps_idxChY[iG_all_grps]
                                                .first])) {
  ...
  for (iG_new = 0; iG_new < Ng_new; iG_new++) {
    if (my_rank == 0 && printOutAll) {
      cout << "&  idxChY_new[iG_new==" << iG_new << "] = ("
           << idxChY_new[iG_new] << " ," << idxChY_new[iG_new] << ")  #  "
           << Nc_vec_new[iG_new] << " = Nc_vec" << endl;
    }
  }

  //##################################################################################
  //#   7.1) remove redundant center groups. I.e. groups that have both a right
  //and  # #        a left neighbor group which sufficiently overlap without the
  //center     # #        group #
  //##################################################################################
  if (my_rank == 0 && printOutAll) {
    cout << endl << "################## 7.1 stated .." << endl;
  }
  ...
  if (rm_cntr_grps) {
    ...
    for (iG_new = 0; iG_new < Ng_new - 2; iG_new++) {
      o_fw(iG_new) =
          idxChY_new[iG_new] + Nc_vec_new[iG_new] - idxChY_new[iG_new + 2];
    ...
    while (o_fw_min_Nc_half_ceil.maxCoeff() > 0. ||
           o_bw_min_Nc_half_ceil.maxCoeff() > 0.) {
      // forward overlapping
      double fw_max = -1;
      for (iG_new = 0; iG_new < Ng_new; iG_new++) {
      ...
      int fw_max_cnt = 0;
      SpEOVectorI I_fw = SpEOVectorI::Zero(
          Ng_new);  // that's a conservative memory allocation. It will contain
                    // only one element in most cases
      for (iG_new = 0; iG_new < Ng_new; iG_new++) {
        if ((o_fw(iG_new) - Nc_half_ceil(iG_new) > 0) &&
            (o_fw(iG_new) / Nc_vec_new[iG_new] > fw_max - myEps)) {
      ...
      int bw_max_cnt = 0;
      SpEOVectorI I_bw = SpEOVectorI::Constant(
          Ng_new, Ng_new);  // that's a conservative memory allocation. It will
                            // contain only one element in most cases
      for (iG_new = 0; iG_new < Ng_new; iG_new++) {
        if ((o_bw(iG_new) - Nc_half_ceil(iG_new) > 0) &&
            (o_bw(iG_new) / Nc_vec_new[iG_new] > bw_max - myEps)) {
      ...
      if (my_rank == 0 && printOutAll) {
        ...
        cout << " |    Ng_new = " << Ng_new << endl << " |    idxChY_new = ";
        ...
        cout << endl
             ...
             << " |    o_fw_min_Nc_half_ceil = "
             << o_fw_min_Nc_half_ceil.transpose() << endl
             << " |   bw_max = " << bw_max << "   ->   I_bw = ";
      ...
      if (bw_max_cnt == 0 && fw_max_cnt == 0) {
        if (my_rank == 0 && printOutAll) {
          cout << "overlapping reduction completed successfully.. "
                  "(bw_max_cnt==0 && fw_max_cnt==0)"
               << endl;
        }
      } else {
        if (my_rank == 0 && printOutAll) {
          cout << "bw_max_cnt = " << bw_max_cnt << ", fw_max_cnt=" << fw_max_cnt
               << endl;
        }
        if (fw_max >= bw_max) {
          int iii = I_fw.maxCoeff();
          if (my_rank == 0 && printOutAll) {
            cout << "   ACTION: fw_max >= bw_max   &&   remove group iii+1="
                 << iii + 1;
          }
          if (my_rank == 0 && printOutAll) {
            cout << "           (idxChY_new[iii+1]=" << idxChY_new[iii + 1]
                 << ", and  Nc_vec_new[iii+1]=" << Nc_vec_new[iii + 1] << ")"
                 << endl;
          }
          // remove group iii+1
  ...
  //##################################################################################
  //#   7.2) Reduce possible high overlapping and redundancy of neighboring
  //groups   # #        by an iterative reduction of groups in terms of both the
  //size reduction # #        and elimination, following Algorithm1. #
  //##################################################################################
  if (my_rank == 0 && printOutAll) {
    cout << "################## 7.2 stated .." << endl;
  }
  ...
  for (iG_new = 0; iG_new < Ng_new - 1; iG_new++) {
    o_fw(iG_new) =
        idxChY_new[iG_new] + Nc_vec_new[iG_new] - idxChY_new[iG_new + 1];
  ...
  while (o_fw_min_Nc_half_ceil.maxCoeff() > 0. ||
    ...
    if (bw_max_cnt == 0 && fw_max_cnt == 0) {
      if (my_rank == 0 && printOutAll) {
        cout << "overlapping reduction completed successfully.. (bw_max_cnt==0 "
                "&& fw_max_cnt==0)"
             << endl;
      }
    } else {
      ...
      if (fw_max >= bw_max) {
        int iii = I_fw.maxCoeff();
        if ((iii > 0) && ((idxChY_new[iii - 1] + Nc_vec_new[iii - 1] -
                           idxChY_new[iii + 1]) == (o_fw(iii) - 1))) {
          if (my_rank == 0 && printOutAll) {
            cout << "   ACTION: fw_max >= bw_max   &&   remove group iii="
                 << iii;
          }
          if (my_rank == 0 && printOutAll) {
            cout << "           (idxChY_new[iii]=" << idxChY_new[iii]
                 << ", and  Nc_vec_new[iii]=" << Nc_vec_new[iii] << ")" << endl;
          }
          // remove group iii
          ...
          if (reduce_larger_grps && iii < Ng_new - 1) {
            // shrink group iii+1
            if (my_rank == 0 && printOutAll) {
              cout << "   ACTION: fw_max >= bw_max   &&   shrink group iii+1="
                   << iii
                   << "+1 by setting  Nc_vec_new[iii+1] -= 1 AND "
                      "idxChY_new[iii+1] += 1";
            }
            if (my_rank == 0 && printOutAll) {
              cout << "           (idxChY_new[iii]=" << idxChY_new[iii]
                   << ", and  Nc_vec_new[iii]=" << Nc_vec_new[iii] << ")"
                   << endl;
            }
            Nc_vec_new[iii + 1] -= 1;
            idxChY_new[iii + 1] += 1;
            ...
            // shrink group iii
            if (my_rank == 0 && printOutAll) {
              cout << "   ACTION: fw_max >= bw_max   &&   shrink group iii="
                   << iii << " by setting  Nc_vec_new[iii] -= 1";
            }
            if (my_rank == 0 && printOutAll) {
              cout << "           (idxChY_new[iii]=" << idxChY_new[iii]
                   << ", and  Nc_vec_new[iii]=" << Nc_vec_new[iii] << ")"
        ...
        int iii = I_bw.minCoeff();
        if ((iii < Ng_new - 1) && ((idxChY_new[iii - 1] + Nc_vec_new[iii - 1] -
                                    idxChY_new[iii + 1]) == (o_bw(iii) - 1))) {
          if (my_rank == 0 && printOutAll) {
            cout << "   ACTION: fw_max < bw_max   &&   remove group iii="
                 << iii;
          }
          ...
          if (reduce_larger_grps && iii > 0) {
            ...
            if (my_rank == 0 && printOutAll) {
              cout << "   ACTION: fw_max < bw_max   &&   shrink group iii-1="
                   << iii << "-1 by setting  Nc_vec_new[iii-1] -= 1";
            }
            if (my_rank == 0 && printOutAll) {
              cout << "           (idxChY_new[iii]=" << idxChY_new[iii]
                   << ", and  Nc_vec_new[iii]=" << Nc_vec_new[iii] << ")"
            ...
            if (my_rank == 0 && printOutAll) {
              cout << "   ACTION: fw_max < bw_max   &&   shrink group iii="
                   << iii
                   << " by setting  Nc_vec_new[iii] -= 1;  AND  "
                      "idxChY_new[iii] += 1;";
            }
            if (my_rank == 0 && printOutAll) {
              cout << "           (idxChY_new[iii]=" << idxChY_new[iii]
  ...
  if (my_rank == 0 && printOutAll) {
    ...
    cout << endl
         << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" << endl
         << "&   after shrinking and possibly removing highly overlapping "
            "groups "
         << endl
         << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" << endl;
    cout << endl
  ...
  for (int iChYU = 0; iChYU < NChY - 1; iChYU++) {
    ...
    while (iChYV < NChY && !jumpToNextU) {
      if (CCmat(iChYU, iChYV) < q + myeps) {
        SpEOMatrixD q_mat_tmp =
            SpEOMatrixD::Constant(iChYU + 1, NChY - iChYV, q);
...
void calc_refined_CCmins(double &CCmin1, double &CCmin2, double &CCmin3,
                         SpEOMatrixD &CCmat_grp, double CCmin_orig,
                         int Nc_max) {
  ...
  //###########################
  int N_sub = N - Nc_max;
  int B = (N_sub * N_sub + N_sub) / 2;  // number of entries in upper half of
                                        // upper sub-matrix incl. "diagonal"
  int desired_num_entries =
      A - B;  // index corresponding to CCmin1 in sorted vec which contains the
              // upper half of CCmin_orig
  ...
  std::sort(CCmat_grp_vec_for_sorting.begin(), CCmat_grp_vec_for_sorting.end(),
            argsort_comp);
...
bool testNNLS(const MatrixType &A,
              const Matrix<typename MatrixType::Scalar,
                           MatrixType::RowsAtCompileTime, 1> &b,
              const Matrix<typename MatrixType::Scalar,
                           MatrixType::ColsAtCompileTime, 1> &x,
              typename MatrixType::Scalar eps = 1e-10) {
  NNLS<MatrixType> nnls(A, 30, eps);
  if (!nnls.solve(b)) {
    std::cerr << __FILE__ << ": Convergence failed!" << std::endl;
    return false;
  }
  ...
  if (!nnls.check(b)) {
    std::cerr << __FILE__ << ": check() KKT returned false!" << std::endl;
    return false;
  }
  ...
  for (iCh = 0; iCh < Im->get_NCh(); iCh++) {
    for (u = 0; u < Im->get_sizeU(); u++) {
      for (v = 0; v < Im->get_sizeV(); v++) {
        uv = u * Im->get_sizeV() + v;
        Im_2D(iCh, uv) =
            (double)(Im->get_rasterBands()[iCh]->get_bandDataMat()->coeff(u,
                                                                          v));
      }
    }
  }
...
bool estimate_SRFs_and_ImX_shift(SpEOMatrixD &SRF, double *&ImX_shift,
  ...
  for (iChX = 0; iChX < NChX; iChX++) {
    ...
    if (my_rank == 0)
      cout << " ImX_shift = " << ImX_shift[iChX] * my_scaling_factor << endl;
  }
