...
void getUserSettings(SpEOPaths *paths, SpEODataIOSetting *dSetting,
  ...
  fSetting->use_estimated_SRFs =
      (bool)atoi(argv[7]);  // estimate_SRFs_from_data
  ...
  //----------------
  // 16: maximum size of spectral group above groups will be double-checked and
  // perhaps split into subgoups (int)
  //----------------
  ...
  //----------------
  fSetting->evaluate =
      (bool)atoi(argv[26]);  // perform_quality_assessment_of_fusion_result

  fSetting->evaluate_ImZ_init = fSetting->evaluate;

  //----------------
  // 27: write fused image in file (1: create file and write resulting image in
  // file; 0: to not write image in file (useful for analyses only)) (bool)
  //----------------
  oSetting->writeImageFile = (bool)atoi(argv[27]);  // writeImageFile

  //----------------
  // write all intermediate image fusion results/images (after every iteration)
  // (bool) options: 0: do not write image in file (useful for analyses only) 1:
  // create file and write resulting image in file CAN BE PUT BACK TO PROGRAM
  // ARGUMENTS IF NEEDED
  //----------------
  oSetting->writeImageFileAfterEveryIter =
      false;  // writeImageFileAfterEveryIter
  ...
  //     (experimenatal) (bool)
  fSetting->balance_ImX_term_coef =
      (bool)atoi(argv[29]);  // balance_ImX_term_coef
  // ---------------
  ...
  // number of processes to work on one sparse reconstruction problem (solver
  // parallelization)
  ...
  // patch parallelization strategy: work stealing (true) or fixed work
  // scheduling (false)
  pSetting->workStealingTurns = -1;

  // calc. coeff. in full image opt. eq. via SNR calc. of ImX and ImY (bool).
  // include SNR normalization to compensate for colored (band-dependend) noise
  // (bool)
  fSetting->SNR_normalization = true;
  //  matrix (dictionary) normalization norm: 0: spectral norm, 1: Frobenious
  //  norm (bool)
  ...
  //#######################################################
  // Choose a solver for the sparse reconstruction (optimization) problem
  // (currently JPFISTA only)
  ...
  // decides whether or not the mean values of Z are set to the same mean values
  // as Y (i.e. either delta_m remains the initial zero vector or it gets
  // updated via least squares) (bool)
  sSetting->fix_delta_m = false;

  // set lambdaZ_ABC to this number only in the first iteration. A low value can
  // be helpful e.g. if the initial image ImZ_init is not very good/trustworthy
  // (double)
  fSetting->lambdaZ_ABC_in_1st_iter = 1.0;

  // use LR (low resolution) patch norm for normalization of corresponding LR
  // and HR patch in coupled dictionaries. If set to 0 the HR nor is used by
  // default. (bool)
  ...
  //			               = 0  => set negative values to zero only at the
  //very end, before writing the final image 	                       = 1  => set negative values to zero
  //only after patch reconstruction 	                       = 2  => set negative values to zero only
  //after full image optimization 	                       = 3  => set negative values to zero both after
  //patch reconstruction and after full image optimization

  // maximum number of iterations in the GS step to solve the least squares
  // problem on the final image level (int)
  sSetting->maxiter_CGLS_im = 1500;  // atoi(argv[19]);
  // error tolerance (double)
  sSetting->tol_r_CGLS_im = (double)atof("1e-12");

  // write all intermediate image fusion results (after every iteration) (1:
  // create file and write resulting image in file; 0: to not write image in
  ...
  if (oSetting->writeImageFile && pSetting->store_patches_tmp_on_drive &&
    ...
    if (my_rank == 0) {
      cout << endl
           << "WARNING: number of processors used for parallel writing "
              "(parWrNumProc) was set to a number larger than the total number "
              "of processors! It got corrected from "
           << pSetting->parWrNumProc << " to " << my_processes << "!" << endl;
    }
    pSetting->parWrNumProc = my_processes;
  }
  if (oSetting->writeImageFile && pSetting->store_patches_tmp_on_drive &&
    ...
    if (my_rank == 0) {
      cout << endl
           << "WARNING: number of processors used for parallel writing "
              "(parWrNumProc) has to be set to a positive number! It got "
              "corrected from "
           << pSetting->parWrNumProc << " to " << tmp << "!" << endl;
    }
