...
double Average_Gradient(SpEODataset *ImZ, int kCh, bool is_ref) {
  ...
  if (is_ref) {
    SpEOMatrixF diffY =
        ImZ->get_rasterBands()[kCh]->get_bandDataMat()->block(
            0, 0, sizeU - 1,
            sizeV - 1)  // get_bandDataMat()->block(0,0,sizeU-1,sizeV-1)
        - ImZ->get_rasterBands()[kCh]->get_bandDataMat()->block(1, 0, sizeU - 1,
                                                                sizeV - 1);
    SpEOMatrixF diffX = ImZ->get_rasterBands()[kCh]->get_bandDataMat()->block(
                            0, 0, sizeU - 1, sizeV - 1) -
                        ImZ->get_rasterBands()[kCh]->get_bandDataMat()->block(
                            0, 1, sizeU - 1, sizeV - 1);
    ...
    SpEOMatrixD diffY =
        ImZ->get_rasterBands()[kCh]->bandDataMatD.block(
            0, 0, sizeU - 1,
            sizeV - 1)  // get_bandDataMat()->block(0,0,sizeU-1,sizeV-1)
        - ImZ->get_rasterBands()[kCh]->bandDataMatD.block(1, 0, sizeU - 1,
                                                          sizeV - 1);
    SpEOMatrixD diffX = ImZ->get_rasterBands()[kCh]->bandDataMatD.block(
                            0, 0, sizeU - 1, sizeV - 1) -
                        ImZ->get_rasterBands()[kCh]->bandDataMatD.block(
                            0, 1, sizeU - 1, sizeV - 1);
    diffY = diffY.cwiseAbs2();
  ...
  for (int y = 0; y < ImZ_ref->get_sizeU(); y++) {
    for (int x = 0; x < ImZ_ref->get_sizeV(); x++) {
      for (int k = 0; k < NChY; k++) {
        spectrumVec_o(k) =
            (double)(ImZ_ref->get_rasterBands()[k]->get_bandDataMat()->coeff(
                y, x));
        spectrumVec_r(k) = ImZ->get_rasterBands()[k]->bandDataMatD.coeff(y, x);
      }
      tmp = spectrumVec_o.dot(spectrumVec_r) /
            (spectrumVec_o.norm() * spectrumVec_r.norm());
      if (tmp > 1)
        tmp = 1.0;
      else if (tmp < -1)
        tmp = -1.0;
...
double Universal_Image_Quality_Index(SpEOMatrixD *ImZ_ref, int kCh,
                                     SpEOMatrixD *ImZ, int lCh) {
  ...
  double stdDev_or =
      (im_o_zrMn.transpose() * im_r_zrMn)(0, 0) / ((double)(numEl - 1));
  double UIQI = (stdDev_or * 2 * mean_o * mean_r * 2 * stdDev_o * stdDev_r) /
                (stdDev_o * stdDev_r * (mean_o * mean_o + mean_r * mean_r) *
                 (stdDev_o * stdDev_o + stdDev_r * stdDev_r));
  return UIQI;
}
...
double Degree_of_Distorion(SpEODataset *ImZ_ref, SpEODataset *ImZ, int kCh) {
  ...
  SpEOMatrixD imDiff =
      (ImZ_ref->get_rasterBands()[kCh]->get_bandDataMat()->cast<double>()) -
...
double Corr_Coef(SpEODataset *ImZ_ref, SpEODataset *ImZ, int kCh) {
  ...
  SpEOMatrixD im_o_zrMn = ImZ_ref->get_rasterBands()[kCh]
                              ->get_bandDataMat()
                              ->cast<double>()
                              .array() -
                          mean_o;  // = d1_prel
  SpEOMatrixD im_r_zrMn =
      ImZ->get_rasterBands()[kCh]->bandDataMatD.array() - mean_r;  // = d2_prel
  im_o_zrMn.resize(numEl, 1);                                      // = d1
  im_r_zrMn.resize(numEl, 1);                                      // = d2
  double CC = (im_o_zrMn.transpose() * im_r_zrMn)(0, 0) /
              (sqrt((im_o_zrMn.transpose() * im_o_zrMn)(0, 0)) *
               sqrt((im_r_zrMn.transpose() * im_r_zrMn)(0, 0)));
  return CC;
}
...
double Root_Mean_Square_Error(SpEODataset *ImZ_ref, SpEODataset *ImZ, int kCh) {
  ...
  double RMSE = imDiff.norm() /
                sqrt((double)(ImZ_ref->get_sizeU() * ImZ_ref->get_sizeV()));
  return RMSE;
};

void PanSharp_Assessment_Eval(SpEODataset *ImZ_ref, SpEODataset *ImZ,
                              SpEODataset *ImY,
  ...
  for (int k = 0; k < NChZ; k++) {
    ImZ_o_mat[k] =
        ImZ_ref->get_rasterBands()[k]->get_bandDataMat()->cast<double>();
    ImZ_r_mat[k] =
        SpEOMatrixD::Map(ImZ->get_rasterBands()[k]->get_bandDataMatD()->data(),
                         ImZ->get_rasterBands()[k]->get_bandDataMatD()->rows(),
                         ImZ->get_rasterBands()[k]->get_bandDataMatD()->cols());
  ...
  for (int k = 0; k < NChZ; k++) {
    ...
    double max_tmp =
        ImZ_ref->get_rasterBands()[k]->get_bandDataMat()->maxCoeff();
  ...
  for (int k = 0; k < NChZ; k++) {
    for (int l = 0; l < NChZ; l++) {
      double UIQI_ImZ_kl =
          Universal_Image_Quality_Index(ImZ_r_mat, k, ImZ_r_mat, l);
      double UIQI_ImY_kl =
          Universal_Image_Quality_Index(ImY_mat, k, ImY_mat, l);
      if (UIQI_ImZ_kl < 0.0) {
        UIQI_ImZ_kl = 0.0;
      }
      if (UIQI_ImY_kl < 0.0) {
