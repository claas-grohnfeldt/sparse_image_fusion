...
void calcGlobalParams(SpEOGlobalParams *glPrms, SpEOPaths *paths,
  ...
  if (((ImX->get_sizeU() / ImY->get_sizeU()) % 1) > tol ||
      ...
      abs(ImX->get_sizeV() / ImY->get_sizeV() -
          ImX->get_sizeU() / ImY->get_sizeU()) > tol) {
    cerr << "\n\n ERROR: Dimension of pan image must be an even multiple of "
            "the dimension of the low-resolution multispectral image!"
         << "\n\n";
    exit(2);
  ...
  if (dSetting->uLFirst < 0) {
    if (my_rank == 0) {
      cout << "\n uLFirst got corrected from uLFirst=" << dSetting->uLFirst
           << " to uLFirst=0!" << endl;
    }
    dSetting->uLFirst = 0;
  }
  if (dSetting->vLFirst < 0) {
    if (my_rank == 0) {
      cout << "\n vLFirst got corrected from vLFirst=" << dSetting->vLFirst
           << " to vLFirst=0!" << endl;
    }
    dSetting->vLFirst = 0;
  }
  if (dSetting->uLLast >= ImY->get_sizeU()) {
    if (my_rank == 0) {
      cout << "\n uLLast got corrected from uLLast=" << dSetting->uLLast
           << " to uLLast=" << ImY->get_sizeU() - 1 << "=ImY->get_sizeU()-1!"
           << endl;
    }
  ...
  if (dSetting->vLLast >= ImY->get_sizeV()) {
    if (my_rank == 0) {
      cout << "\n vLLast got corrected from vLLast=" << dSetting->vLLast
           << " to vLLast=" << ImY->get_sizeV() - 1 << "=ImY->get_sizeV()-1!"
           << endl;
    }
  ...
  glPrms->NPU = ceil(double((ImY->get_sizeU() - fSetting->patchsize)) / a) +
                1;  // number of patches in a column
  glPrms->NPV = ceil(double((ImY->get_sizeV() - fSetting->patchsize)) / a) +
                1;  // number of patches in a row
  glPrms->NP = glPrms->NPU * glPrms->NPV;

  glPrms->uPFirst = dSetting->uLFirst / a;
  if (glPrms->uPFirst >= glPrms->NPU) {
    glPrms->uPFirst = glPrms->NPU - 1;
  }
  glPrms->uPLast = dSetting->uLLast / a;
  if (glPrms->uPLast >= glPrms->NPU) {
    glPrms->uPLast = glPrms->NPU - 1;
  }
  glPrms->vPFirst = dSetting->vLFirst / a;
  if (glPrms->vPFirst >= glPrms->NPV) {
    glPrms->vPFirst = glPrms->NPV - 1;
  }
  glPrms->vPLast = dSetting->vLLast / a;
  if (glPrms->vPLast >= glPrms->NPV) {
    glPrms->vPLast = glPrms->NPV - 1;
  }

  if (glPrms->uPFirst < 0 || glPrms->uPFirst >= glPrms->NPU) {
    if (my_rank == 0) {
      cout << "\n WARNING: uPFirst got corrected from uPFirst="
           << glPrms->uPFirst << " to uPFirst=0" << endl;
    }
    glPrms->uPFirst = 0;
  }
  if (glPrms->uPLast < 0 || glPrms->uPLast < glPrms->uPFirst ||
      glPrms->uPLast >= glPrms->NPU) {
    if (my_rank == 0) {
      cout << "\n WARNING: uPLast got corrected from uPLast=" << glPrms->uPLast
           << " to uPLast=" << glPrms->NPU << "==NPU!" << endl;
    }
    glPrms->uPLast = glPrms->NPU - 1;
  }
  if (glPrms->vPFirst < 0 || glPrms->vPFirst >= glPrms->NPV) {
    if (my_rank == 0) {
      cout << "\n WARNING: vPFirst got corrected from vPFirst="
           << glPrms->vPFirst << " to vPFirst=0" << endl;
    }
    glPrms->vPFirst = 0;
  }
  if (glPrms->vPLast < 0 || glPrms->vPLast < glPrms->vPFirst ||
      glPrms->vPLast >= glPrms->NPV) {
    if (my_rank == 0) {
      cout << "\n WARNING: vPLast got corrected from vPLast=" << glPrms->vPLast
           << " to vPLast=" << glPrms->NPV << "==NPV!" << endl;
    }
    glPrms->vPLast = glPrms->NPV - 1;
  }
  ...
  if (fSetting->NDP > glPrms->NP) {
    if (my_rank == 0) {
      cout << "\nNDP got corrected from NDP=" << fSetting->NDP
           << " to NDP=" << glPrms->NP << "==NP!" << endl;
    }
...
double l1l2norm(SpEOMatrixD x) { return x.rowwise().norm().lpNorm<1>(); }
...
double spec_norm(SpEOMatrixD A) {
  MatrixXd S =
      A *
      A.transpose();  // use selfadjoint property of spectral norm and use the
                      // computationally more efficient way (smaller matrix)
  ...
  if (!es.info()) {
    ...
    // cout << "     Eigenvalue computation for spectral norm normalization did
    // not converge. Using static value for normalization instead." << endl;
  }
