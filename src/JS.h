...
enum Stopping_criterion { FUNCTIONAL_DIFFERENCE, FIRST_ORDER_CONDITIONS };
...
enum Backtracking_strategy { NO, DEC, DECPAR, DECEFF };

enum Stepsize_update { INIT, LARGE, PSCL };

struct FBSsolverOptions {
  FBSsolverOptions(int _Ndecomp, int _maxiter_in, int _maxiter_out, double _tol,
                   Stopping_criterion _stop_crit, Prediction_rule _pred_rule,
                   Backtracking_strategy _backtracking,
                   Stepsize_update _stepsize_update, bool _stepsize_adaptive,
                   bool _silent, double _gamma)
      : Ndecomp(_Ndecomp),
  ...
  FBSsolverOptions()
      : Ndecomp(1),
        ...
        maxiter_out(10000),
        tol(1e-2),
        stop_crit(FIRST_ORDER_CONDITIONS),
        pred_rule(STANDARD),
        backtracking(NO),
        stepsize_update(INIT),
        stepsize_adaptive(false),
        silent(false),
        gamma(1e2) {}
...
// FBS Solver (parallel)
void FBSSolver(int& iter_need, double& rel_res, SpEOMatrixD& X, int N_all,
...
class Eq1Op {
 public:
  Eq1Op(SpEOMatrixD* R, SpEOMatrixD gauss_filter, SpEOMatrixD filter_coeff,
        int fDS, double coeff1, double coeff2, double coeff3);
...
class Eq1MVar {
 public:
  ...
  void SubtractOperatorXvar_sub(Eq1Op op, SpEOMatrixD* var, MPI_Comm eq1_comm,
                                SpEOMatrixD& EndmemberMat,
                                SpEOVectorD& LAMBDA_ZY);
  void SetOperatorXvar(Eq1Op op, Eq1SVar* var, MPI_Comm eq1_comm);
  void SetOperatorXvar(Eq1Op op, Eq1SVar_sub* var_sub, MPI_Comm eq1_comm,
                       SpEOMatrixD& EndmemberMat);
  void SetOperatorXvar(Eq1Op op, Eq1SVar_sub* var_sub, MPI_Comm eq1_comm,
                       SpEOMatrixD& EndmemberMat, SpEOVectorD& LAMBDA_ZY);
...
class Eq7Op {
 public:
  Eq7Op(SpEOVectorD* dH, SpEOVectorD* dL, SpEOMatrixD* R,
        SpEOVectorD* P_lmd_vecs, SpEOMatrixI* P_lmd_idx_row, int** P_lmd_idx_bl,
...
class Eq9Op {
 public:
  Eq9Op(SpEOMatrixD* DH, SpEOMatrixD* DL, SpEOMatrixD* R,
        SpEOVectorD* P_lmd_vecs, SpEOMatrixI* P_lmd_idx_row, int** P_lmd_idx_bl,
        double* coeff1, double* coeff2, double coeff3);
...
void multiply_2D_with_3D_mat(SpEOMatrixD*& Mat_3D_output, SpEOMatrixD& Mat_2D,
                             SpEOMatrixD*& var_tmp, bool transpose_2D_mat);
