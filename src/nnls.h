...
//NNLS(const MatrixType &A, int max_iter=-1, Scalar eps=1e-10)  // original line
  NNLS(const MatrixType &A, int max_iter=1e4, Scalar eps=1e-10)  // modified by C.G.
...
bool NNLS<MatrixType>::solve(const ColVectorType &b, Heuristic heuristic)
{
  ...
  while (true)
  {
//    std::cout << "itercnt (outer) = " << itercnt++ << std::endl; // added by C.G. temporarily
    // Update gradient _w
    _updateGradient();

    //if(itercnt<50 || itercnt%100==0) std::cout << "itercnt=" << itercnt << "_A.cols()=" << _A.cols() << "(_Np="<<_Np<<")" <<  ", _max_Z(_w=)=" << _max_Z(_w) << " (eps="<<_epsilon<<")" << std::endl; // added by C.G. temporarily
    //if(itercnt<70 || itercnt%100==0) std::cout << "(" << itercnt << ") " << _max_Z(_w) << " "; // added by C.G. temporarily
    // Check if system is solved:
  //if ((_A.cols()==_Np) || ((_max_Z(_w)-_epsilon)<0)) { return true; } // original line
  if ((_A.cols()==_Np) || ((_max_Z(_w)-_epsilon)<0)   || itercnt==500) { return true; } // modified by C.G.
  //if ((_A.cols()==_Np) || ((_max_Z(_w)-_epsilon)<0)  || itercnt==500) { std::cout << "NNLS took " << itercnt << " iters. _max_Z(_w)=" << _max_Z(_w) << " (eps=" << _epsilon<< ")"; return true; } // modified by C.G.
    ...
    while (true)
    {
      //std::cout << "itercnt (outer) = " << itercnt << ", itercnt (inner) = " << itercntInner++ << std::endl; // added by C.G. temporarily
