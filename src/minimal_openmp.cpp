...
int main(int argc, char* argv[]) {
  ...
  if (my_global_rank < numProcBusy) {
...
#ifndef _OPENMP
    // Definition of groups of tasks which work on the same process (similarly
    // to threads in OpenMP)d
    MPI_Barrier(comm_busy);
    MPI_Comm comm_patch;
    MPI_Group group_patch;
    MPI_Comm_group(comm_busy, &group_patch);
    int ranges[1][3] = {{(my_rank / subprocesses) * subprocesses,
                         (my_rank / subprocesses + 1) * subprocesses - 1, 1}};
    ...
    if (workStealingTurns >= 0) {
      lastFixedPatch -=
          (pLast_sub + 1) % p_groups + workStealingTurns * p_groups;
