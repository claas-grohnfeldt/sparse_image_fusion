...
int h12_(integer *mode, integer *lpivot, integer *l1, const integer *m,
         doublereal *u, integer *iue, doublereal *up, doublereal *c__,
         integer *ice, const integer *icv, integer *ncv);
doublereal diff_(doublereal *x, doublereal *y);
int g1_(doublereal *a, doublereal *b, doublereal *cterm, doublereal *sterm,
        doublereal *sig);
...
/* Subroutine */ int nnls_(doublereal *a, const integer *mda, const integer *m,
                           const integer *n, doublereal *b, doublereal *x,
...
L60:
  ...
  if (diff_(&d__2, &unorm) > 0.) {
    ...
    i__1 = npp1 + 1;
    h12_(&c__2, &npp1, &i__1, m, &a[j * a_dim1 + 1], &c__1, &up, &zz[1], &c__1,
         &c__1, &c__1);
...
L260:
  ...
  if (jj != nsetp) {
    ...
    for (j = jj; j <= i__1; ++j) {
      ii = index[j];
      index[j - 1] = ii;
      g1_(&a[j - 1 + ii * a_dim1], &a[j + ii * a_dim1], &cc, &ss,
          &a[j - 1 + ii * a_dim1]);
      a[j + ii * a_dim1] = 0.;
      i__2 = *n;
      for (l = 1; l <= i__2; ++l) {
...
/* Subroutine */ int h12_(integer *mode, integer *lpivot, integer *l1,
                          const integer *m, doublereal *u, integer *iue,
                          doublereal *up, doublereal *c__, integer *ice,
                          const integer *icv, integer *ncv) {
  /* System generated locals */
