...
void create_Gaussian_filter(SpEOMatrixD &filter, int filter_size) {
    ...
    if (filter_size%2 == 0) {
      ...
      for (i = 0; i < radius; i++) {
	for (j = i; j < radius; j++) {
	  filter(radius + i, radius + j) = exp(-(pow(i+0.5,2)+pow(j+0.5,2))*kappa);
...
void apply_filter(SpEOMatrixD &HRp, SpEOMatrixD filter, int fDS) { // only square filters
  ...
  if (filter_size%2 == 0) {
	...
	double filteredpatch_sum = (HRp_tmp.block(i+lm,j+tm,rm-lm+1,bm-tm+1).cwiseProduct(filter.block(radius-1+lm,radius-1+tm,rm-lm+1,bm-tm+1))).sum();
...
void apply_filter_adjoint(SpEOMatrixD &HRp, SpEOMatrixD filter, int fDS) { // only square filters
  int i,j;
  ...
  if (filter_size%2 == 0) {
	...
	int tm = max(-j, -radius + 1);
	int bm = min(radius, N_j-(j+1));	
	if (i >= 0 && j >= 0) HRp_tmp.block(i+lm,j+tm,rm-lm+1,bm-tm+1) += HRp(i,j)*filter.block(radius-1+lm,radius-1+tm,rm-lm+1,bm-tm+1);
	if (i+1 < N_i && j >= 0) HRp_tmp.block(i+lm,j+tm,rm-lm+1,bm-tm+1) += HRp(i+1,j)*filter.block(radius-1+lm,radius-1+tm,rm-lm+1,bm-tm+1);
	if (j+1 < N_j && i >= 0) HRp_tmp.block(i+lm,j+tm,rm-lm+1,bm-tm+1) += HRp(i,j+1)*filter.block(radius-1+lm,radius-1+tm,rm-lm+1,bm-tm+1);
	if (i+1 < N_i && j+1 < N_j) HRp_tmp.block(i+lm,j+tm,rm-lm+1,bm-tm+1) += HRp(i+1,j+1)*filter.block(radius-1+lm,radius-1+tm,rm-lm+1,bm-tm+1);
      }
    }
    HRp = HRp_tmp;
  }
...
void select(SpEOMatrixD &LRp, SpEOMatrixD HRp, int fDS) {
  ...
  for (i = 0; i < LR_rows; i++) {
    for (j = 0; j < LR_cols; j++) {
      LRp(i,j) = HRp(i*fDS + fDS/2, j*fDS + fDS/2); // lower right corner of center 2x2 square if fDS even -> ok for Gaussian filter and assumption that dimensions of HRp are multiple of fDS
    }
  }
...
void distribute(SpEOMatrixD &HRp, SpEOMatrixD LRp, int fDS) { //adjoint operator of "select"
  int i,j;
...
void calc_filter_boundary_coeff(SpEOMatrixD &filter_coeff, SpEOMatrixD filter, int fDS) { //assume filter.sum() == 1
  int i,j;
  ...
  for (i = 0; i < table_size; i++) {
    for (j = i; j < table_size; j++) {
      filter_coeff(i,j) = 1.0/filter.block(i,j,filter_size - i, filter_size - j).sum();
...
void apply_filter_boundary_coeff(SpEOMatrixD &HRp, SpEOMatrixD filter_coeff, int filter_size, int fDS) { // filter_size even <=> image sizes even
  int i,j;
  ...
  if (filter_size%2 == 0) {
    ...
    for (i = start_index; i < HR_rows; i= i+2) {
      int i_overlap = max(0,max(max_overlap - i, max_overlap - (HR_rows-(i+2))));
      for (j = start_index; j < HR_cols; j= j+2) {
	int j_overlap = max(0,max(max_overlap - j, max_overlap - (HR_cols-(j+2))));
	if (i_overlap > 0 || j_overlap > 0) {
	  if (i >= 0 && j >= 0) HRp(i,j) *= filter_coeff(i_overlap,j_overlap);
	  if (i+1 < HR_rows && j >= 0) HRp(i+1,j) *= filter_coeff(i_overlap,j_overlap);
	  if (j+1 < HR_cols && i >= 0) HRp(i,j+1) *= filter_coeff(i_overlap,j_overlap);
	  if (i+1 < HR_rows && j+1 < HR_cols) HRp(i+1,j+1) *= filter_coeff(i_overlap,j_overlap);
	}
      }
    }
  }
    ...
    for (i = 0; i < HR_rows; i++) {
      int i_overlap = max(0,max(max_overlap - i, max_overlap - (HR_rows-(i+1))));
      for (j = 0; j < HR_cols; j++) {
	int j_overlap = max(0,max(max_overlap - j, max_overlap - (HR_cols-(j+1))));
	if (i_overlap > 0 || j_overlap > 0) {
...
void fast_filter(SpEOMatrixD &LRp, SpEOMatrixD HRp, SpEOMatrixD filter, SpEOMatrixD filter_coeff, int fDS) {
  ...
  if (filter_size%2 == 0) {
    ...
    for (i_LR = 0; i_LR < LR_rows; i_LR++) {
      ...
      int i_overlap = max(0,max(max_overlap - i_HR, max_overlap - (HR_rows-(i_HR+2))));
      for (j_LR = 0; j_LR < LR_cols; j_LR++) {
	j_HR = fDS*j_LR + fDS/2 - 1;
	int tm = max(-j_HR, -radius + 1);
	int bm = min(radius, HR_cols-(j_HR+1));
	int j_overlap = max(0,max(max_overlap - j_HR, max_overlap - (HR_cols-(j_HR+2))));
	LRp(i_LR,j_LR) = filter_coeff(i_overlap,j_overlap)*(HRp.block(i_HR+lm,j_HR+tm,rm-lm+1,bm-tm+1).cwiseProduct(filter.block(radius-1+lm,radius-1+tm,rm-lm+1,bm-tm+1))).sum();
      }
    }
  }
  else {
    ...
    for (i_LR = 0; i_LR < LR_rows; i_LR++) {
      ...
      int rm = min(radius, HR_rows-(i_HR+1));
      int i_overlap = max(0,max(max_overlap - i_HR, max_overlap - (HR_rows-(i_HR+1))));
      for (j_LR = 0; j_LR < LR_cols; j_LR++) {
	j_HR = fDS*j_LR + fDS/2;
	int tm = max(-j_HR, -radius);
	int bm = min(radius, HR_cols-(j_HR+1));
	int j_overlap = max(0,max(max_overlap - j_HR, max_overlap - (HR_cols-(j_HR+1))));
	LRp(i_LR,j_LR) = filter_coeff(i_overlap,j_overlap)*(HRp.block(i_HR+lm,j_HR+tm,rm-lm+1,bm-tm+1).cwiseProduct(filter.block(radius+lm,radius+tm,rm-lm+1,bm-tm+1))).sum();
      }
    }
  }
  return;
