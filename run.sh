...
#DIR_DATA_BASE='./data/HS_MS/ROSIS_Pavia_Univeristy'
...
$reference_image_available $filename_ImZ_ref $estimate_SRFs_from_data \
$filename_SRF $dirname_output $patch_size $patch_overlap $N_a \
$dictselect $lambda $theta $N_c $mu_X $mu_Y $ImX_sim_mode $winSize \
$mu_X_prime $mu_Y_prime $subspace_dim $processing_module_flag \
$iterMain $perform_quality_assessment_of_fusion_result \
$writeImageFile $output_image_format_flag $balance_ImX_term_coef"

# PROG_ARGS="$ARG_job_name $LOADL_PID $datasetID $alg $lambda $useSimulatedImXforDictLearn $ImX_sim_mode $dictselect $N_a $psz $patch_overlap $mu_X $mu_Y $N_c $theta $winSize $mu_X_prime $mu_Y_prime $maxiter_globalOpt $tol_r_globalOpt $fullImOptOnSubspace $subspace_transform_type $subspace_dim $SNR_normalization $balance_ImX_term_coef $use_estimated_SRFs $ImZ_init_type $use_global_proc_module $use_ONLY_global_proc_module $iterMain $eval $evaluate_ImZ_init $writeImageFile $writeImageFileAfterEveryIter $saveAsDouble $filename_ImX $filename_ImY $filename_ImZ_init $filename_ImZ_ref $filename_SRF $dir_out $dir_tmp"
